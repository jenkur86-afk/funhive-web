...
- **Age-range detection: don't reimplement `detectAgeRange()` locally in a scraper.** As of 2026-08-03, `supabase-adapter.js`'s save-time logic (`data.ageRange || detectAgeRange(name, description) || normalizeAgeRange(name)`) is the authoritative pipeline — it's richer than any per-scraper keyword check (grade patterns like "Gr 4-8"/"K-1st Grade", numeric ranges, many synonym forms) and falls back to normalizing the raw title itself before giving up to "All Ages". A scraper setting its own `data.ageRange` short-circuits this — only do it if you have a genuinely structured source field (e.g. an API's own `ageText`/`minAge`/`maxAge`), and pass that raw value straight through rather than reducing it through a local keyword match first; `normalizeAgeRange()` parses raw strings like "6-12" or "18+" natively. Five scraper files (`scraper-recdesk-parks.js`, `scraper-drupal-parks.js`, `scraper-nps-parks.js`, `scraper-activenet-parks-eastern.js`, `scraper-civicrec-parks-eastern.js`) had their own weaker local detectors removed on this date after a DB audit found 84.2% of all events tagged "All Ages" platform-wide, disproportionately concentrated in this parks/rec family — see `SCRAPER-FIX-LOG.jsonl` for the full trace and `AGE-RANGE-AUDIT.md` (built by the funhive-scraper-diagnosis scheduled task, Step 3c) for the ongoing per-site breakdown. If you're diagnosing a high All-Ages% on some other scraper, check first whether it has its own local `detectAgeRange`/similar function before assuming the shared pipeline is at fault.
