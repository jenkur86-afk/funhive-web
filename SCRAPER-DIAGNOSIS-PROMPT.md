...
- **WordPress-{state} library scrapers with a persistently high InvalidDate rate (especially 25%+)**: check whether the site runs WordPress's "The Events Calendar" (TEC) plugin before assuming it needs another DOM-parsing patch — look for `?post_type=tribe_events` / `?eventDisplay=list` in the site's URLs. TEC's list view groups events under a bare day heading like "Tue 4" with no month/year on the heading itself (only once at the top of the page), which no ancestor-DOM-walk can recover a full date from. `scrapers/helpers/tec-rest-helper.js` (added 2026-07-31) sidesteps this entirely by hitting the site's `wp-json/tribe/events/v1/events` REST API for clean structured dates, falling back to DOM scraping only on non-TEC sites. It's already wired into `scraper-wordpress-libraries-{ny,fl,nj,ms,me,ga,nc,ct,tn,al,vt}.js` — before adding it to another state file, `grep -l tryFetchTecEvents scrapers/scraper-wordpress-libraries-*.js` to see which ones still need it, and check `SCRAPER-FIX-LOG.jsonl` for whether that state's high InvalidDate rate was already diagnosed as this same TEC pattern rather than re-investigating from scratch. GA/NC/CT/TN/AL/VT were wired in on 2026-08-01 but not yet re-run to confirm — if their InvalidDate counts in a run dated 2026-08-01 or earlier still look high (GA ~46%, NC ~43%, AL ~43%, TN ~32%, CT ~29%, VT ~13%), that's the pre-fix baseline, not a fresh unresolved problem; only treat it as still-broken if a run genuinely dated *after* the fix commit shows the same rates.
...
- **Age-range precedence (2026-08-04): a scraper-supplied `ageRange` only wins when it is actually specific.** Both save paths now go through `resolveAgeRange(data)` in `supabase-adapter.js` instead of the old `data.ageRange || detectAgeRange(...)` short-circuit. The old form let any scraper-supplied value win outright, including low-confidence generic labels — the WordPress-{state} parser accepts any `[class*="category"]` chip as its age element, so a site's "All Ages" category tag beat completely unambiguous titles. Verified against live rows: `Toddler Time Downtown (Ages 18-36 Months)` and `Baby Bounce (Ages 0-18 Months)` were both stored as All Ages. `resolveAgeRange()` keeps a specific supplied bracket (including `Adults`, which callers depend on for adult-only rejection) but re-checks the title/description via `detectAgeRange()` whenever the supplied value normalizes to the All Ages catch-all. Three bucketing/ordering bugs were fixed in the same pass, all caused by `normalizeAgeRange()` bucketing on a range's **lower bound**: bare "teen" returned `11-18` → Tweens (now `13-18`), generic "kids"/"children" returned `4-12` → Preschool (now `6-12`), "elementary" returned `5-11` → Preschool (now `6-11`); and the grade-pattern block was moved ahead of the generic kids/elementary keywords, since "for kids from 3rd grade to 6th grade" was matching the generic rule first and losing the grade range entirely. Plural keyword misses were also fixed (`teens`, `preschoolers`, `infants` all previously fell through to All Ages because of trailing `\b` anchors). Regression suite: `scripts/test-age-detection.js` — run it after any change here.
- **Age-range detection: don't reimplement `detectAgeRange()` locally in a scraper, and never feed a raw uncurated title/description into `normalizeAgeRange()` directly.** As of 2026-08-03, `supabase-adapter.js`'s `detectAgeRange(name, description)` is the authoritative extraction pass — it now also recognizes grade-level phrasing ("Gr 4-8", "K-1st Grade", "6-8th Grade", "Grades 3-5") in addition to its existing "ages X-Y"/parenthetical/keyword checks, converting grade numbers to an approximate age range (grade N ≈ age N+5) itself before handing off to `normalizeAgeRange()`. Do NOT try to "fix" a low-detection scraper by piping its raw name straight into `normalizeAgeRange()` as a fallback — that was tried and reverted the same day it shipped: `normalizeAgeRange()`'s generic numeric-range regex has no context awareness, so it happily misreads a time ("1:00 - 2:30p" → "0-2"), a registration-ID/year pair ("2608-2026" → "8-20"), or a skill level as an age range. `detectAgeRange()`'s checks are deliberately anchored (require "ages"/parens/"grade" context) specifically to avoid that — extend those anchored checks, don't bypass them. Also watch bare keyword checks for false positives from venue names, not just numbers: the pre-existing `elementary` keyword rule was firing on "held at Lincoln Elementary" venue mentions before being tightened to require an actual audience qualifier ("elementary students/age/kids"). Five scraper files (`scraper-recdesk-parks.js`, `scraper-drupal-parks.js`, `scraper-nps-parks.js`, `scraper-activenet-parks-eastern.js`, `scraper-civicrec-parks-eastern.js`) had their own weaker local detectors removed in favor of this shared pipeline; ActiveNet/CivicRec now pass their source APIs' structured `ageText` field straight through as `data.ageRange` (skip local processing entirely when you genuinely have a structured field like that — `normalizeAgeRange()` parses raw values like "6-12" or "18+" natively). All of this was driven by a DB audit that found 84.2% of all events tagged "All Ages" platform-wide, disproportionately concentrated in the parks/rec family — see `SCRAPER-FIX-LOG.jsonl` for the full trace (including the false-positive rounds and how they were caught) and `AGE-RANGE-AUDIT.md` (built by the funhive-scraper-diagnosis scheduled task, Step 3c) for the ongoing per-site breakdown. `scripts/backfill-age-range.js` re-derives `age_range` for existing "All Ages" rows using this same logic — re-run it (dry-run first) if the detection logic changes again. If you're diagnosing a high All-Ages% on some other scraper, check first whether it has its own local `detectAgeRange`/similar function before assuming the shared pipeline is at fault.
- Data quality runs on a tiered cadence (Apr 2026):
  - **Daily**: `node scripts/data-quality-quick.js` (count-only audit, ~5 MB) and `.\scripts\fix-all.ps1 --recent-only` (last 24h, ~15–50 MB). Git Bash: `bash scripts/fix-all.sh --recent-only`.
