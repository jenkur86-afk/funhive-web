...
- **`OSM_SCRAPERS` in `scraper-registry.js` (11 entries, `OSM-Batch1`–`OSM-Batch10` + `OSM-California`) point at `scraper-osm-batch-activities.js` and `scraper-osm-california.js`, neither of which exists in the repo** — confirmed via `git log`, they've never existed since the initial commit. These are placeholder registry entries for a feature that was never built, not a regression. `--osm` is a real CLI flag on `local-scraper-runner.js` but nothing calls it automatically, and running it would just throw "Scraper file not found" for every batch. Don't spend time debugging "why don't OSM scrapers produce results" — either build the two scraper files for real or remove the dead registry entries; there's nothing to "fix" in the wiring.
