...
- **Active state counts (2026-09-02)**: Group 1 = 52, Group 2 = 52, Group 3 = 53 (157 active of 227 registered; the other 70 are inactive-region entries). Recompute with `isScraperActive(scraper, getActiveStates())` over `SCRAPERS` rather than trusting this line — it drifts as scrapers are added.
- **Groups are balanced by MEASURED RUNTIME, not by count (rebalanced 2026-09-02).** The counts above are near-equal only incidentally; what is actually equalised is wall-clock time, because that is what overruns a 24h trigger. Before the rebalance the load was **Group 1 = 30.1h, Group 2 = 24.7h, Group 3 = 12.6h** — Group 1 alone exceeded its own trigger interval, which is why it kept eating the following day's rotation. After: **22.4h / 22.4h / 22.5h**, and split across the two tasks that now run them, **regular 7.6h each** and **MacaroniKid 14.8h each** (was 19.3 / 17.1 / 8.0 — MacaroniKid's 9/7/4 state split was most of the imbalance).
  - Assignment method: median observed `TIME(s)` per scraper from `scrapers/logs/scraper-summary.log`, then greedy longest-processing-time-first bin packing. Timing coverage was 157/157 regular and 20/20 MacaroniKid, so nothing was estimated.
  - **Re-run the balance after adding or removing scrapers**, and after any change that materially alters a scraper's runtime. A group drifting back toward 24h is how the overrun defect returns. Inactive-region entries are deliberately NOT balanced — they never run and have no timing data.
  - Only the `group:` field was changed; all 270 registry keys and every other field are byte-identical, verified against the previous commit.
...
- MacaroniKid scrapers: 43 state-specific files (`scraper-macaroni-{2-letter-state}.js`, e.g. `scraper-macaroni-al.js`), each with identical structure. They run through **separate** runner files (`scrapers/macaroni-runner-group{1,2,3}.js`), one per rotation group, not through `local-scraper-runner.js`'s per-scraper loop directly.
  - **2026-04-15 to 2026-08-04 incident**: `local-scraper-runner.js`'s `runMacaroniGroup()` was a dead stub for ~4 months — it only logged instructions to run the group file manually and never actually invoked it, and no scheduled task called the group runners either. All 43 states of MacaroniKid coverage silently went stale except one manual catch-up on 2026-07-09. Fixed 2026-08-04 by spawning the group runner as the rotation's tail — which then caused the rotation-starvation defect below.
  - **Since 2026-08-31 MacaroniKid runs as its own scheduled task** (`FunHive-Macaroni` → `run-macaroni.bat` → `macaroni-daily-runner.js`), NOT as part of `local-scraper-runner.js`'s rotation. As the rotation's tail it pushed Group 1 runs to ~26h against a 24h trigger and silently deleted the following day's rotation (see `ROTATION-STARVATION-LOG.md`). If you ever see zero `MacaroniKid-*` activity in `scraper-summary.log` for an extended period again, check the `FunHive-Macaroni` task (`Get-ScheduledTaskInfo -TaskName FunHive-Macaroni`) and `scrapers/logs/macaroni-last-run.json` — the preflight's `macaroni task running` check watches exactly this. Do not re-wire MacaroniKid back into the rotation.
...
**Five** things run on a schedule — four via Windows Task Scheduler, one via Claude Code. All four Task Scheduler entries are registered by `scrapers/task-scheduler/setup-tasks.ps1` (run once, elevated); that script is the source of truth for their settings and asserts them on read-back.
...
| `FunHive-Macaroni` | daily 3:00 PM | `scrapers/run-macaroni.bat` | 30h | `scrapers/logs/macaroni-stdout.log`, `macaroni-stderr.log`, `macaroni-daily-<date>.log`, `macaroni-last-run.json` |
...
- **`FunHive-Scrapers`** — runs the day's rotation group, **regular scrapers only as of 2026-08-31**. Writes the cumulative per-scraper table (FOUND/NEW/DUPES/INVALID/TIME) to `scrapers/logs/scraper-summary.log`.
  - **It no longer runs MacaroniKid. Do not re-chain it here.** MacaroniKid was this task's tail until 2026-08-31, which made Group 1 runs ~26h against a 24h trigger — and because the task is `MultipleInstances=IgnoreNew` with `StartWhenAvailable=False`, every overrun silently DISCARDED the next day's rotation (2026-08-26 and 2026-08-29 had no rotation at all; Group 2 ran every 5–6 days instead of 3). Full history and the intervention ledger: `ROTATION-STARVATION-LOG.md`.
- **`FunHive-Macaroni`** — runs one MacaroniKid group per day via `scrapers/macaroni-daily-runner.js` (calendar group + its own starvation catch-up against `scrapers/logs/macaroni-last-run.json`). It serializes against the rotation on `scrapers/logs/runner.lock` (`scrapers/helpers/run-lock.js`): whichever full-group runner starts second **waits, bounded at 8h, logging why** — it is never silently dropped. A long lock wait in the logs is designed behavior, not a hang. Completion is recorded from the group runner's fresh results file, not its exit code, so one failed state doesn't mark the whole group starved but a mid-run crash does.
  - **The two scraper tasks must never share a stdout/stderr capture file, and the run lock does not protect this.** They shared `scraper-stdout.log` from the 2026-08-31 split until 2026-09-02, when the first real overlap silently deleted a whole rotation: `cmd.exe` opens a `>>` target at process start and will not share it with a second `cmd` process, so `run-scrapers.bat` could not open its redirect while `run-macaroni.bat` held the file — its first `echo` failed, **node never launched**, and the task exited `LastTaskResult = 1` with no `scraper-run-<date>.log` at all. `run-lock.js` lives inside node and so cannot help: the rotation dies before it can wait. MacaroniKid now captures to `macaroni-stdout.log` / `macaroni-stderr.log`, and `scripts/build-library-site-audit.js` parses **both** files by default (its `DEFAULT_LOGS`) so no per-site detail is lost. If either file is ever renamed, update `DEFAULT_LOGS` in the same commit.
