...
/**
 * WISCONSIN MACARONI KID SCRAPER
 *
 * Scrapes events from 2 Wisconsin Macaroni Kid sites
 *
 * Coverage: Appleton-Waupaca-Oshkosh, Madison
 *
 * Usage:
 *   node scraper-macaroni-wi.js          # Test mode (5 sites)
 *   node scraper-macaroni-wi.js --full   # Production (all 2 sites)
 */
...
// All 2 Wisconsin Macaroni Kid Sites
const WI_MK_SITES = [
  { url: 'https://appleton.macaronikid.com', name: 'Appleton-Waupaca-Oshkosh', county: 'Outagamie' },
  { url: 'https://madisonwi.macaronikid.com', name: 'Madison', county: 'Dane' }
];

async function geocodeAddress(address, city, zipCode) {
  return _sharedGeocodeAddress(address, city, 'WI', zipCode);
}

async function geocodeVenue(venue, city, zipCode) {
  return _sharedGeocodeVenue(venue, city, 'WI', zipCode);
}
    ...
    for (const url of eventUrls) {
      ...
      if (coords) {
        ...
        if (!coords && details.city) {
          coords = await getCityCenterCoords(details.city, 'WI', details.zipCode);
          if (coords) {
            locationObj = {
              address: details.address || '',
              city: details.city,
        ...
        if (!coords) {
          const countyCentroid = getCountyCentroid(site.county, 'WI');
          if (countyCentroid) {
      ...
      const eventDoc = {
        ...
        metadata: {
          source: 'Macaroni Kid Wisconsin Scraper',
          sourceUrl: site.url,
          scraperName: `MacaroniKid-WI-${site.url.replace(/^https?:\/\//, '').replace(/^www\./, '').split('.')[0]}`,
...
async function scrapeMacaroniKidWisconsin() {
  const fullMode = true;
  console.log('🥕 WISCONSIN MACARONI KID SCRAPER\n' + '='.repeat(60));
  const sitesToScrape = fullMode ? WI_MK_SITES : WI_MK_SITES.slice(0, 5);
  ...
  console.log('\n' + '='.repeat(60));
  console.log('✅ WISCONSIN MACARONI KID SCRAPER COMPLETE!');
  console.log(`📊 Summary:`);
  ...
  await logScraperResult('Macaroni Kid WI', {
    new: imported,
    duplicates: skipped,
    errors: failed
  }, { state: 'WI', source: 'macaroni-kid' });

  return { imported, failed };
}

if (require.main === module) {
  scrapeMacaroniKidWisconsin()
    .then(() => process.exit(0))
    .catch(error => {
...
module.exports = { scrapeMacaroniKidWisconsin };
