...
/**
 * RHODE ISLAND MACARONI KID SCRAPER
 *
 * Scrapes events from 3 Rhode Island Macaroni Kid sites
 *
 * Coverage: Cranston-Kent, East Bay-Newport, Southern Rhode Island
 *
 * Usage:
 *   node scraper-macaroni-ri.js          # Test mode (5 sites)
 *   node scraper-macaroni-ri.js --full   # Production (all 3 sites)
 */
...
// All 2 Rhode Island Macaroni Kid Sites (East Bay-Newport retired)
const RI_MK_SITES = [
  { url: 'https://kent.macaronikid.com', name: 'Cranston-Kent', county: 'Providence' },
  // REMOVED: East Bay-Newport — redirects to national.macaronikid.com (site retired)
  // { url: 'https://newport.macaronikid.com', name: 'East Bay-Newport', county: 'Newport' },
  { url: 'https://washingtonri.macaronikid.com', name: 'Southern Rhode Island', county: 'Washington' }
];

async function geocodeAddress(address, city, zipCode) {
  return _sharedGeocodeAddress(address, city, 'RI', zipCode);
}

async function geocodeVenue(venue, city, zipCode) {
  return _sharedGeocodeVenue(venue, city, 'RI', zipCode);
}
    ...
    for (const url of eventUrls) {
      ...
      if (coords) {
        ...
        if (!coords && details.city) {
          coords = await getCityCenterCoords(details.city, 'RI', details.zipCode);
          if (coords) {
            locationObj = {
              address: details.address || '',
              city: details.city,
        ...
        if (!coords) {
          const countyCentroid = getCountyCentroid(site.county, 'RI');
          if (countyCentroid) {
      ...
      const eventDoc = {
        ...
        metadata: {
          ...
          sourceUrl: site.url,
          scraperName: `MacaroniKid-RI-${site.url.replace(/^https?:\/\//, '').replace(/^www\./, '').split('.')[0]}`,
...
async function scrapeMacaroniKidRhodeIsland() {
  const fullMode = true;
  console.log('🥕 RHODE ISLAND MACARONI KID SCRAPER\n' + '='.repeat(60));
  const sitesToScrape = fullMode ? RI_MK_SITES : RI_MK_SITES.slice(0, 5);
  const maxEventsPerSite = fullMode ? 999 : 30; // No limit - filtered by 60 day date range
  console.log(`Mode: ${fullMode ? 'FULL (all 3 sites)' : 'LIMITED (5 sites)'}\nDate range: Next 60 days\n`);
  ...
  console.log('\n' + '='.repeat(60));
  console.log('✅ RHODE ISLAND MACARONI KID SCRAPER COMPLETE!');
  console.log(`📊 Summary:`);
  ...
  await logScraperResult('Macaroni Kid RI', {
    new: imported,
    duplicates: skipped,
    errors: failed
  }, { state: 'RI', source: 'macaroni-kid' });

  return { imported, failed };
}

if (require.main === module) {
  scrapeMacaroniKidRhodeIsland()
    .then(() => process.exit(0))
    .catch(error => {
...
module.exports = { scrapeMacaroniKidRhodeIsland };
