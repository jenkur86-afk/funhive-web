...
/**
 * ALASKA MACARONI KID SCRAPER
 *
 * Scrapes events from 2 Alaska Macaroni Kid sites
 *
 * Coverage: Anchorage-Eagle River, Wasilla
 *
 * Usage:
 *   node scraper-macaroni-ak.js          # Test mode (all 2 sites)
 *   node scraper-macaroni-ak.js --full   # Production (all 2 sites)
 */
...
// All 2 Alaska Macaroni Kid Sites
const ALASKA_MK_SITES = [
  { url: 'https://anchorage.macaronikid.com', name: 'Anchorage-Eagle River', county: 'Anchorage' },
  { url: 'https://wasilla.macaronikid.com', name: 'Wasilla', county: 'Matanuska-Susitna' }
];

async function geocodeAddress(address, city, zipCode) {
  const fullAddress = `${address}, ${city}, AK ${zipCode}`;
  let result = await tryGeocode(fullAddress);
  if (result) return result;

  const cleaned = address.replace(/,?\s*Suite\s+[A-Z0-9-]+/i, '').replace(/,?\s*#\s*[A-Z0-9-]+/i, '');
  if (cleaned !== address) {
    result = await tryGeocode(`${cleaned}, ${city}, AK ${zipCode}`);
    if (result) return result;
  }

  const streetOnly = cleaned.split(',')[0];
  return await tryGeocode(`${streetOnly}, AK ${zipCode}`);
}
  ...
  try {
    ...
    return await page.evaluate(() => {
      ...
      if (eventsIndex > -1) {
        ...
        if (idx < lines.length) { const cityZipMatch = lines[idx].match(/^([\w\s]+)\s+AK\s+(\d{5})/); if (cityZipMatch) { result.city = cityZipMatch[1].trim(); result.zipCode = cityZipMatch[2]; }}
      }
  ...
  try {
    ...
    for (const url of eventUrls) {
      ...
      const eventDoc = {
        ...
        metadata: {
          source: 'Macaroni Kid Alaska Scraper',
...
async function scrapeMacaroniKidAlaska() {
  const fullMode = true; // Use full mode since only 2 sites
  console.log('🥕 ALASKA MACARONI KID SCRAPER\n' + '='.repeat(60));
  const sitesToScrape = ALASKA_MK_SITES; // Always scrape both sites
  const maxEventsPerSite = 50;
  console.log(`Mode: FULL (all 2 sites)\nDate range: Next 60 days\n`);
  ...
  console.log('\n' + '='.repeat(60));
  console.log('✅ ALASKA MACARONI KID SCRAPER COMPLETE!');
  console.log(`📊 Summary:`);
  ...
  await logScraperResult('Macaroni Kid AK', {
    new: imported,
    duplicates: skipped,
    errors: failed
  }, { state: 'AK', source: 'macaroni-kid' });
...
module.exports = { scrapeMacaroniKidAlaska };
