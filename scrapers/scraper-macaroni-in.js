...
async function geocodeAddress(address, city, zipCode) {
  return _sharedGeocodeAddress(address, city, 'IN', zipCode);
}

async function geocodeVenue(venue, city, zipCode) {
  return _sharedGeocodeVenue(venue, city, 'IN', zipCode);
}
    ...
    for (const url of eventUrls) {
      ...
      if (coords) {
        ...
        if (!coords && details.city) {
          coords = await getCityCenterCoords(details.city, 'IN', details.zipCode);
          if (coords) {
            locationObj = {
              address: details.address || '',
              city: details.city,
      ...
      const eventDoc = {
        ...
        metadata: {
          ...
          sourceUrl: site.url,
          scraperName: `MacaroniKid-IN-${site.url.replace(/^https?:\/\//, '').replace(/^www\./, '').split('.')[0]}`,
