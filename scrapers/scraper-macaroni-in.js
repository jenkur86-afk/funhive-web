...
/**
 * INDIANA MACARONI KID SCRAPER
 *
 * Scrapes events from 4 Indiana Macaroni Kid sites
 *
 * Coverage: Evansville, Hendricks, Jeffersonville-Clarksville-New Albany, Lafayette, Indiana
 *
 * Usage:
 *   node scraper-macaroni-in.js          # Test mode (5 sites)
 *   node scraper-macaroni-in.js --full   # Production (all 4 sites)
 */
...
// All 4 Indiana Macaroni Kid Sites
const IN_MK_SITES = [
  { url: 'https://evansville.macaronikid.com', name: 'Evansville', county: 'Vanderburgh' },
  { url: 'https://hendricks.macaronikid.com', name: 'Hendricks', county: 'Hendricks' },
  { url: 'https://jeffersonville.macaronikid.com', name: 'Jeffersonville-Clarksville-New Albany', county: 'Clark' },
  { url: 'https://lafayette.macaronikid.com', name: 'Lafayette, Indiana', county: 'Tippecanoe' }
];

async function geocodeAddress(address, city, zipCode) {
  const fullAddress = `${address}, ${city}, IN ${zipCode}`;
  let result = await tryGeocode(fullAddress);
  if (result) return result;

  const cleaned = address.replace(/,?\s*Suite\s+[A-Z0-9-]+/i, '').replace(/,?\s*#\s*[A-Z0-9-]+/i, '');
  if (cleaned !== address) {
    result = await tryGeocode(`${cleaned}, ${city}, IN ${zipCode}`);
    if (result) return result;
  }

  const streetOnly = cleaned.split(',')[0];
  return await tryGeocode(`${streetOnly}, IN ${zipCode}`);
}
  ...
  try {
    ...
    return await page.evaluate(() => {
      ...
      if (eventsIndex > -1) {
        ...
        if (idx < lines.length) { const cityZipMatch = lines[idx].match(/^([\w\s]+)\s+IN\s+(\d{5})/); if (cityZipMatch) { result.city = cityZipMatch[1].trim(); result.zipCode = cityZipMatch[2]; }}
      }
  ...
  try {
    ...
    for (const url of eventUrls) {
      ...
      if (coords) {
        ...
        if (!coords) {
          const countyCentroid = getCountyCentroid(site.county, 'IN');
          if (countyCentroid) {
      ...
      const eventDoc = {
        ...
        metadata: {
          source: 'Macaroni Kid Indiana Scraper',
...
async function scrapeMacaroniKidIndiana() {
  const fullMode = true;
  console.log('🥕 INDIANA MACARONI KID SCRAPER\n' + '='.repeat(60));
  const sitesToScrape = fullMode ? IN_MK_SITES : IN_MK_SITES.slice(0, 5);
  ...
  console.log('\n' + '='.repeat(60));
  console.log('✅ INDIANA MACARONI KID SCRAPER COMPLETE!');
  console.log(`📊 Summary:`);
  ...
  await logScraperResult('Macaroni Kid IN', {
    new: imported,
    duplicates: skipped,
    errors: failed
  }, { state: 'IN', source: 'macaroni-kid' });

  return { imported, failed };
}

if (require.main === module) {
  scrapeMacaroniKidIndiana()
    .then(() => process.exit(0))
    .catch(error => {
...
module.exports = { scrapeMacaroniKidIndiana };
