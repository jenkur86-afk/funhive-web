...
/**
 * IDAHO MACARONI KID SCRAPER
 *
 * Scrapes events from 3 Idaho Macaroni Kid sites
 *
 * Coverage: Boise, Meridian, Nampa
 *
 * Usage:
 *   node scraper-macaroni-id.js          # Test mode (5 sites)
 *   node scraper-macaroni-id.js --full   # Production (all 3 sites)
 */
...
// All 3 Idaho Macaroni Kid Sites
const ID_MK_SITES = [
  { url: 'https://boise.macaronikid.com', name: 'Boise', county: 'Ada' },
  { url: 'https://meridian.macaronikid.com', name: 'Meridian', county: 'Ada' },
  { url: 'https://nampa.macaronikid.com', name: 'Nampa', county: 'Canyon' }
];

async function geocodeAddress(address, city, zipCode) {
  const fullAddress = `${address}, ${city}, ID ${zipCode}`;
  let result = await tryGeocode(fullAddress);
  if (result) return result;

  const cleaned = address.replace(/,?\s*Suite\s+[A-Z0-9-]+/i, '').replace(/,?\s*#\s*[A-Z0-9-]+/i, '');
  if (cleaned !== address) {
    result = await tryGeocode(`${cleaned}, ${city}, ID ${zipCode}`);
    if (result) return result;
  }

  const streetOnly = cleaned.split(',')[0];
  return await tryGeocode(`${streetOnly}, ID ${zipCode}`);
}
  ...
  try {
    ...
    return await page.evaluate(() => {
      ...
      if (eventsIndex > -1) {
        ...
        if (idx < lines.length) { const cityZipMatch = lines[idx].match(/^([\w\s]+)\s+ID\s+(\d{5})/); if (cityZipMatch) { result.city = cityZipMatch[1].trim(); result.zipCode = cityZipMatch[2]; }}
      }
  ...
  try {
    ...
    for (const url of eventUrls) {
      ...
      if (coords) {
        ...
        if (!coords) {
          const countyCentroid = getCountyCentroid(site.county, 'ID');
          if (countyCentroid) {
      ...
      const eventDoc = {
        ...
        metadata: {
          source: 'Macaroni Kid Idaho Scraper',
...
async function scrapeMacaroniKidIdaho() {
  const fullMode = true;
  console.log('🥕 IDAHO MACARONI KID SCRAPER\n' + '='.repeat(60));
  const sitesToScrape = fullMode ? ID_MK_SITES : ID_MK_SITES.slice(0, 5);
  ...
  console.log('\n' + '='.repeat(60));
  console.log('✅ IDAHO MACARONI KID SCRAPER COMPLETE!');
  console.log(`📊 Summary:`);
  ...
  await logScraperResult('Macaroni Kid ID', {
    new: imported,
    duplicates: skipped,
    errors: failed
  }, { state: 'ID', source: 'macaroni-kid' });

  return { imported, failed };
}

if (require.main === module) {
  scrapeMacaroniKidIdaho()
    .then(() => process.exit(0))
    .catch(error => {
...
module.exports = { scrapeMacaroniKidIdaho };
