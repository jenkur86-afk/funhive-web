    ...
    for (const url of eventUrls) {
      ...
      if (coords) {
        ...
        if (!coords && details.city) {
          coords = await tryGeocode(`${details.city}, DC ${details.zipCode || ''}`);
          if (coords) {
            locationObj = {
              address: details.address || '',
              city: details.city,
