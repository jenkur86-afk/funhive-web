...
/**
 * WASHINGTON DC MACARONI KID SCRAPER
 *
 * Scrapes events from 1 District of Columbia Macaroni Kid site
 *
 * Coverage: DC East/Capitol Hill
 *
 * Usage:
 *   node scraper-macaroni-dc.js          # Test mode (5 sites)
 *   node scraper-macaroni-dc.js --full   # Production (all 1 site)
 */
...
async function geocodeAddress(address, city, zipCode) {
  return _sharedGeocodeAddress(address, city, 'DC', zipCode);
}

async function geocodeVenue(venue, city, zipCode) {
  return _sharedGeocodeVenue(venue, city, 'DC', zipCode);
}
...
async function scrapeSite(browser, site, maxEvents = 50) {
  ...
  try {
    ...
    for (const url of eventUrls) {
      ...
      // Always skip promotional newsletter-type events
      if (isPromoName) {
        console.log(`  ⏭️ Skipping promo: ${(details.name || '').substring(0, 40)}...`);
        continue;
      }

      // Only skip online-venue events if they also lack a physical address
      if (isOnlineVenue && !hasPhysicalLocation) {
        console.log(`  ⏭️ Skipping online (no address): ${(details.name || '').substring(0, 40)}...`);
        continue;
      }

      // Check for online-only indicators in description (only if no physical location)
      ...
      if (coords) {
        ...
        if (!coords && details.city) {
          coords = await getCityCenterCoords(details.city, 'DC', details.zipCode);
          if (coords) {
            locationObj = {
              address: details.address || '',
              city: details.city,
        ...
        if (!coords) {
          const countyCentroid = getCountyCentroid(site.county, 'DC');
          if (countyCentroid) {
      ...
      const eventDoc = {
        ...
        metadata: {
          source: 'Macaroni Kid DC Scraper',
          sourceUrl: site.url,
          scraperName: `MacaroniKid-DC-${site.url.replace(/^https?:\/\//, '').replace(/^www\./, '').split('.')[0]}`,
...
async function scrapeMacaroniKidDC() {
  const fullMode = true;
  console.log('🏛️  WASHINGTON DC MACARONI KID SCRAPER\n' + '='.repeat(60));
  const sitesToScrape = fullMode ? DC_MK_SITES : DC_MK_SITES.slice(0, 5);
  const maxEventsPerSite = fullMode ? 999 : 30; // No limit - filtered by 60 day date range
  console.log(`Mode: ${fullMode ? 'FULL (all 1 site)' : 'LIMITED (5 sites)'}\nDate range: Next 60 days\n`);
  ...
  console.log('\n' + '='.repeat(60));
  console.log('✅ WASHINGTON DC MACARONI KID SCRAPER COMPLETE!');
  console.log(`📊 Summary:`);
  ...
  await logScraperResult('Macaroni Kid DC', {
    new: imported,
    duplicates: skipped,
    errors: failed
...
module.exports = { scrapeMacaroniKidDC };
