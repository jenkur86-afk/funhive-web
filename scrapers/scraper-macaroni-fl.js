...
/**
 * FLORIDA MACARONI KID SCRAPER
 *
 * Scrapes events from 31 Florida Macaroni Kid sites
 *
 * Coverage: Apollo Beach-Ruskin-Wimauma, Apopka-Mount Dora, Boynton Beach-Delray Beach, Bradenton, Brandon, Cape Coral, Clearwater, Daytona Beach, Fort Lauderdale, Fort Myers-Fort Myers Beach, Lakeland-Bartow-Mulberry, Miami East-Watson Island to Coral Gables, New Tampa, North Brevard, Okeechobee-Loxahatchee, Palm Beach Gardens-Jupiter, Port Charlotte-Punta Gorda, Port St Lucie, Sarasota-Venice, Sebring-Lake Placid-Avon Park, Seminole County, South Brevard, South Lake-Celebration-Clermont, Stuart, Vero Beach, Wellington-Royal Palm Beach-Greenacres, West Palm Beach-Lake Worth-Lantana, West Volusia, Winter Garden-Ocoee, Winter Haven-Davenport, Winter Park
 *
 * Usage:
 *   node scraper-macaroni-fl.js          # Test mode (5 sites)
 *   node scraper-macaroni-fl.js --full   # Production (all 31 sites)
 */
...
// All 31 Florida Macaroni Kid Sites
const FL_MK_SITES = [
  { url: 'https://apollobeach.macaronikid.com', name: 'Apollo Beach-Ruskin-Wimauma', county: 'Hillsborough' },
  { url: 'https://apopka.macaronikid.com', name: 'Apopka-Mount Dora', county: 'Orange' },
  { url: 'https://boyntonbeach.macaronikid.com', name: 'Boynton Beach-Delray Beach', county: 'Palm Beach' },
  { url: 'https://bradenton.macaronikid.com', name: 'Bradenton', county: 'Manatee' },
  { url: 'https://brandon.macaronikid.com', name: 'Brandon', county: 'Hillsborough' },
  { url: 'https://capecoral.macaronikid.com', name: 'Cape Coral', county: 'Lee' },
  { url: 'https://clearwater.macaronikid.com', name: 'Clearwater', county: 'Pinellas' },
  { url: 'https://daytonabeach.macaronikid.com', name: 'Daytona Beach', county: 'Volusia' },
  { url: 'https://fortlauderdale.macaronikid.com', name: 'Fort Lauderdale', county: 'Broward' },
  { url: 'https://fortmyersbeach.macaronikid.com', name: 'Fort Myers-Fort Myers Beach', county: 'Lee' },
  { url: 'https://lakeland.macaronikid.com', name: 'Lakeland-Bartow-Mulberry', county: 'Polk' },
  { url: 'https://pinecrest.macaronikid.com', name: 'Miami East-Watson Island to Coral Gables', county: 'Miami-Dade' },
  { url: 'https://newtampafl.macaronikid.com', name: 'New Tampa', county: 'Hillsborough' },
  { url: 'https://northbrevard.macaronikid.com', name: 'North Brevard', county: 'Brevard' },
  { url: 'https://okeechobee.macaronikid.com', name: 'Okeechobee-Loxahatchee', county: 'Okeechobee' },
  { url: 'https://pbgjupiter.macaronikid.com', name: 'Palm Beach Gardens-Jupiter', county: 'Palm Beach' },
  { url: 'https://portcharlotte.macaronikid.com', name: 'Port Charlotte-Punta Gorda', county: 'Charlotte' },
  { url: 'https://portstlucie.macaronikid.com', name: 'Port St Lucie', county: 'St. Lucie' },
  { url: 'https://sarasota.macaronikid.com', name: 'Sarasota-Venice', county: 'Sarasota' },
  { url: 'https://sebring.macaronikid.com', name: 'Sebring-Lake Placid-Avon Park', county: 'Highlands' },
  { url: 'https://seminolecounty.macaronikid.com', name: 'Seminole County', county: 'Seminole' },
  { url: 'https://melbourne.macaronikid.com', name: 'South Brevard', county: 'Brevard' },
  { url: 'https://southlake.macaronikid.com', name: 'South Lake-Celebration-Clermont', county: 'Lake' },
  { url: 'https://stuart.macaronikid.com', name: 'Stuart', county: 'Martin' },
  { url: 'https://verobeach.macaronikid.com', name: 'Vero Beach', county: 'Indian River' },
  { url: 'https://wellington.macaronikid.com', name: 'Wellington-Royal Palm Beach-Greenacres', county: 'Palm Beach' },
  { url: 'https://westpalmbeach.macaronikid.com', name: 'West Palm Beach-Lake Worth-Lantana', county: 'Palm Beach' },
  { url: 'https://deltona.macaronikid.com', name: 'West Volusia', county: 'Volusia' },
  { url: 'https://wintergarden.macaronikid.com', name: 'Winter Garden-Ocoee', county: 'Orange' },
  { url: 'https://winterhaven-davenport.macaronikid.com', name: 'Winter Haven-Davenport', county: 'Polk' },
  { url: 'https://winterpark.macaronikid.com', name: 'Winter Park', county: 'Orange' }
];
...
async function geocodeAddress(address, city, zipCode) {
  ...
  if (cleaned !== address) {
    result = await tryGeocode(`${cleaned}, ${city}, FL ${zipCode}`);
    if (result) return result;
  }

  const streetOnly = cleaned.split(',')[0];
  return await tryGeocode(`${streetOnly}, FL ${zipCode}`);
}
  ...
  try {
    ...
    return await page.evaluate(() => {
      ...
      if (eventsIndex > -1) {
        ...
        if (idx < lines.length) { const cityZipMatch = lines[idx].match(/^([\w\s]+)\s+FL\s+(\d{5})/); if (cityZipMatch) { result.city = cityZipMatch[1].trim(); result.zipCode = cityZipMatch[2]; }}
      }
  ...
  try {
    ...
    for (const url of eventUrls) {
      ...
      if (coords) {
        ...
        if (!coords) {
          const countyCentroid = getCountyCentroid(site.county, 'FL');
          if (countyCentroid) {
      ...
      const eventDoc = {
        ...
        metadata: {
          source: 'Macaroni Kid Florida Scraper',
...
async function scrapeMacaroniKidFlorida() {
  const fullMode = true; // PRODUCTION: All sites
  console.log('🥕 FLORIDA MACARONI KID SCRAPER\n' + '='.repeat(60));
  const sitesToScrape = fullMode ? FL_MK_SITES : FL_MK_SITES.slice(0, 5);
  const maxEventsPerSite = fullMode ? 999 : 30; // No limit - filtered by 60 day date range
  console.log(`Mode: ${fullMode ? 'FULL (all 31 sites)' : 'LIMITED (5 sites)'}\nDate range: Next 60 days\n`);
  ...
  console.log('\n' + '='.repeat(60));
  console.log('✅ FLORIDA MACARONI KID SCRAPER COMPLETE!');
  console.log(`📊 Summary:`);
  ...
  await logScraperResult('Macaroni Kid FL', {
    new: imported,
    duplicates: skipped,
    errors: failed
  }, { state: 'FL', source: 'macaroni-kid' });

  return { imported, failed };
}

if (require.main === module) {
  scrapeMacaroniKidFlorida()
    .then(() => process.exit(0))
    .catch(error => {
...
module.exports = { scrapeMacaroniKidFlorida };
