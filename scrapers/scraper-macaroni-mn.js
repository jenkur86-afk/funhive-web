...
async function geocodeAddress(address, city, zipCode) {
  return _sharedGeocodeAddress(address, city, 'MN', zipCode);
}

async function geocodeVenue(venue, city, zipCode) {
  return _sharedGeocodeVenue(venue, city, 'MN', zipCode);
}
    ...
    for (const url of eventUrls) {
      ...
      if (coords) {
        ...
        if (!coords && details.city) {
          coords = await getCityCenterCoords(details.city, 'MN', details.zipCode);
          if (coords) {
            locationObj = {
              address: details.address || '',
              city: details.city,
      ...
      const eventDoc = {
        ...
        metadata: {
          ...
          sourceUrl: site.url,
          scraperName: `MacaroniKid-MN-${site.url.replace(/^https?:\/\//, '').replace(/^www\./, '').split('.')[0]}`,
