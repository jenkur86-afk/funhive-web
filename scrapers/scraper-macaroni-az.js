...
/**
 * ARIZONA MACARONI KID SCRAPER
 *
 * Scrapes events from 8 Arizona Macaroni Kid sites
 *
 * Coverage: Chandler, Gilbert, Glendale-Luke AFB, North Phoenix,
 *           North Scottsdale-PV, Queen Creek-San Tan Valley,
 *           South Scottsdale-Arcadia, Surprise-Peoria-El Mirage
 *
 * Usage:
 *   node scraper-macaroni-az.js          # Test mode (5 sites)
 *   node scraper-macaroni-az.js --full   # Production (all 8 sites)
 */
...
// All 8 Arizona Macaroni Kid Sites
const ARIZONA_MK_SITES = [
  { url: 'https://chandler.macaronikid.com', name: 'Chandler', county: 'Maricopa' },
  { url: 'https://gilbert.macaronikid.com', name: 'Gilbert', county: 'Maricopa' },
  { url: 'https://glendaleaz.macaronikid.com', name: 'Glendale-Luke AFB', county: 'Maricopa' },
  { url: 'https://nphoenix.macaronikid.com', name: 'North Phoenix', county: 'Maricopa' },
  { url: 'https://nscottsdale.macaronikid.com', name: 'North Scottsdale-PV', county: 'Maricopa' },
  { url: 'https://queencreek.macaronikid.com', name: 'Queen Creek-San Tan Valley', county: 'Maricopa' },
  { url: 'https://southscottsdale.macaronikid.com', name: 'South Scottsdale-Arcadia', county: 'Maricopa' },
  { url: 'https://surprise.macaronikid.com', name: 'Surprise-Peoria-El Mirage', county: 'Maricopa' }
];

async function geocodeAddress(address, city, zipCode) {
  const fullAddress = `${address}, ${city}, AZ ${zipCode}`;
  let result = await tryGeocode(fullAddress);
  if (result) return result;

  const cleaned = address.replace(/,?\s*Suite\s+[A-Z0-9-]+/i, '').replace(/,?\s*#\s*[A-Z0-9-]+/i, '');
  if (cleaned !== address) {
    result = await tryGeocode(`${cleaned}, ${city}, AZ ${zipCode}`);
    if (result) return result;
  }

  const streetOnly = cleaned.split(',')[0];
  return await tryGeocode(`${streetOnly}, AZ ${zipCode}`);
}
  ...
  try {
    ...
    return await page.evaluate(() => {
      ...
      if (eventsIndex > -1) {
        ...
        if (idx < lines.length) { const cityZipMatch = lines[idx].match(/^([\w\s]+)\s+AZ\s+(\d{5})/); if (cityZipMatch) { result.city = cityZipMatch[1].trim(); result.zipCode = cityZipMatch[2]; }}
      }
  ...
  try {
    ...
    for (const url of eventUrls) {
      ...
      if (coords) {
        ...
        const countyCentroid = getCountyCentroid(site.county, 'AZ');
        if (countyCentroid) {
      ...
      const eventDoc = {
        ...
        metadata: {
          source: 'Macaroni Kid Arizona Scraper',
...
async function scrapeMacaroniKidArizona() {
  const fullMode = true; // PRODUCTION: All sites // Use limited mode for Cloud Functions
  console.log('🥕 ARIZONA MACARONI KID SCRAPER\n' + '='.repeat(60));
  const sitesToScrape = fullMode ? ARIZONA_MK_SITES : ARIZONA_MK_SITES.slice(0, 5);
  ...
  console.log('\n' + '='.repeat(60));
  console.log('✅ ARIZONA MACARONI KID SCRAPER COMPLETE!');
  console.log(`📊 Summary:`);
  ...
  await logScraperResult('Macaroni Kid AZ', {
    new: imported,
    duplicates: skipped,
    errors: failed
...
module.exports = { scrapeMacaroniKidArizona };
