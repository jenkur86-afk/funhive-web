...
/**
 * OREGON MACARONI KID SCRAPER
 *
 * Scrapes events from 2 Oregon Macaroni Kid sites
 *
 * Coverage: Eugene, Salem
 *
 * Usage:
 *   node scraper-macaroni-or.js          # Test mode (5 sites)
 *   node scraper-macaroni-or.js --full   # Production (all 2 sites)
 */
...
// All 2 Oregon Macaroni Kid Sites
const OR_MK_SITES = [
  { url: 'https://eugene.macaronikid.com', name: 'Eugene', county: 'Lane' },
  { url: 'https://salem.macaronikid.com', name: 'Salem', county: 'Marion' }
];

async function geocodeAddress(address, city, zipCode) {
  const fullAddress = `${address}, ${city}, OR ${zipCode}`;
  let result = await tryGeocode(fullAddress);
  if (result) return result;

  const cleaned = address.replace(/,?\s*Suite\s+[A-Z0-9-]+/i, '').replace(/,?\s*#\s*[A-Z0-9-]+/i, '');
  if (cleaned !== address) {
    result = await tryGeocode(`${cleaned}, ${city}, OR ${zipCode}`);
    if (result) return result;
  }

  const streetOnly = cleaned.split(',')[0];
  return await tryGeocode(`${streetOnly}, OR ${zipCode}`);
}
  ...
  try {
    ...
    return await page.evaluate(() => {
      ...
      if (eventsIndex > -1) {
        ...
        if (idx < lines.length) { const cityZipMatch = lines[idx].match(/^([\w\s]+)\s+OR\s+(\d{5})/); if (cityZipMatch) { result.city = cityZipMatch[1].trim(); result.zipCode = cityZipMatch[2]; }}
      }
  ...
  try {
    ...
    for (const url of eventUrls) {
      ...
      if (coords) {
        ...
        if (!coords) {
          const countyCentroid = getCountyCentroid(site.county, 'OR');
          if (countyCentroid) {
      ...
      const eventDoc = {
        ...
        metadata: {
          source: 'Macaroni Kid Oregon Scraper',
...
async function scrapeMacaroniKidOregon() {
  const fullMode = true;
  console.log('🥕 OREGON MACARONI KID SCRAPER\n' + '='.repeat(60));
  const sitesToScrape = fullMode ? OR_MK_SITES : OR_MK_SITES.slice(0, 5);
  ...
  console.log('\n' + '='.repeat(60));
  console.log('✅ OREGON MACARONI KID SCRAPER COMPLETE!');
  console.log(`📊 Summary:`);
  ...
  await logScraperResult('Macaroni Kid OR', {
    new: imported,
    duplicates: skipped,
    errors: failed
  }, { state: 'OR', source: 'macaroni-kid' });

  return { imported, failed };
}

if (require.main === module) {
  scrapeMacaroniKidOregon()
    .then(() => process.exit(0))
    .catch(error => {
...
module.exports = { scrapeMacaroniKidOregon };
