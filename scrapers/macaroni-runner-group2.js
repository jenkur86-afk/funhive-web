...
/**
 * MACARONI KID RUNNER - GROUP 2
 * Runs MacaroniKid scrapers for Group 2 states only
 *
 * Schedule: Days 2, 5, 8, 11, 14, 17, 20, 23, 26, 29
 *
 * Usage:
 *   node macaroni-runner-group2.js                 # Run all Group 2 states
 *   node macaroni-runner-group2.js --dry-run       # Preview without running
 *   node macaroni-runner-group2.js --state TX      # Run specific state
 *
...
// See macaroni-runner-group1.js for rationale.
// ----------------------------------------------------------------------------
...
const CONFIG = {
  ...
  // Log file
  LOG_FILE: path.join(__dirname, '..', 'logs', `macaroni-group2-${new Date().toISOString().split('T')[0]}.log`)
};
...
async function logToSupabase(scraperName, status, stats, error, executionTime) {
  try {
    await saveScraperLog({
      scraperName: `MacaroniKid-Group2-${scraperName}`,
...
async function main() {
  ...
  if (options.help) {
    console.log(`
MacaroniKid Runner - Group 2

Usage:
  node macaroni-runner-group2.js                 # Run all Group 2 states
  node macaroni-runner-group2.js --state TX      # Run specific state
  node macaroni-runner-group2.js --dry-run       # Preview without running
  node macaroni-runner-group2.js --help          # Show this help

Schedule: Days 2, 5, 8, 11, 14, 17, 20, 23, 26, 29

Group 2 States: ${Object.values(MACARONI_SCRAPERS)
