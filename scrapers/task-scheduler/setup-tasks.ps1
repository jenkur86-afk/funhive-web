...
Write-Host "run-scrapers.bat redirects into logs\scraper-stdout.log / scraper-stderr.log;"
Write-Host "run-macaroni.bat redirects into logs\macaroni-stdout.log / macaroni-stderr.log;"
Write-Host "run-fix-all.bat redirects into logs\fix-all-recent.log."
Write-Host "The two scraper tasks MUST NOT share a capture file: cmd.exe will not share a"
Write-Host "'>>' target between processes, so on 2026-09-02 the 3:00 AM rotation could not"
Write-Host "open its redirect while FunHive-Macaroni held it, and exited 1 without ever"
Write-Host "launching node - the whole rotation was lost. build-library-site-audit.js reads"
Write-Host "both capture files, so the split costs no per-site detail."
