...
/**
 * LOUISIANA MACARONI KID SCRAPER
 *
 * Scrapes events from 9 Louisiana Macaroni Kid sites
 *
 * Coverage: Acadia-St Landry, Houma, Lafayette, Monroe-West Monroe, New Iberia-Abbeville-Breaux Bridge, New Orleans, Shreveport-Bossier, St Tammany, West Bank
 *
 * Usage:
 *   node scraper-macaroni-la.js          # Test mode (5 sites)
 *   node scraper-macaroni-la.js --full   # Production (all 9 sites)
 */
...
// All 9 Louisiana Macaroni Kid Sites
const LA_MK_SITES = [
  { url: 'https://acadia.macaronikid.com', name: 'Acadia-St Landry', county: 'Acadia' },
  { url: 'https://houma.macaronikid.com', name: 'Houma', county: 'Terrebonne' },
  { url: 'https://lafayettela.macaronikid.com', name: 'Lafayette', county: 'Lafayette' },
  { url: 'https://monroela.macaronikid.com', name: 'Monroe-West Monroe', county: 'Ouachita' },
  { url: 'https://newiberia.macaronikid.com', name: 'New Iberia-Abbeville-Breaux Bridge', county: 'Iberia' },
  { url: 'https://new-orleans.macaronikid.com', name: 'New Orleans', county: 'Orleans' },
  { url: 'https://shreveport.macaronikid.com', name: 'Shreveport-Bossier', county: 'Caddo' },
  { url: 'https://sttammany.macaronikid.com', name: 'St Tammany', county: 'St. Tammany' },
  { url: 'https://westbank.macaronikid.com', name: 'West Bank', county: 'Jefferson' }
];

async function geocodeAddress(address, city, zipCode) {
  const fullAddress = `${address}, ${city}, LA ${zipCode}`;
  let result = await tryGeocode(fullAddress);
  if (result) return result;

  const cleaned = address.replace(/,?\s*Suite\s+[A-Z0-9-]+/i, '').replace(/,?\s*#\s*[A-Z0-9-]+/i, '');
  if (cleaned !== address) {
    result = await tryGeocode(`${cleaned}, ${city}, LA ${zipCode}`);
    if (result) return result;
  }

  const streetOnly = cleaned.split(',')[0];
  return await tryGeocode(`${streetOnly}, LA ${zipCode}`);
}
  ...
  try {
    ...
    return await page.evaluate(() => {
      ...
      if (eventsIndex > -1) {
        ...
        if (idx < lines.length) { const cityZipMatch = lines[idx].match(/^([\w\s]+)\s+LA\s+(\d{5})/); if (cityZipMatch) { result.city = cityZipMatch[1].trim(); result.zipCode = cityZipMatch[2]; }}
      }
  ...
  try {
    ...
    for (const url of eventUrls) {
      ...
      if (coords) {
        ...
        if (!coords) {
          const countyCentroid = getCountyCentroid(site.county, 'LA');
          if (countyCentroid) {
      ...
      const eventDoc = {
        ...
        metadata: {
          source: 'Macaroni Kid Louisiana Scraper',
...
async function scrapeMacaroniKidLouisiana() {
  const fullMode = true; // PRODUCTION: All sites
  console.log('🥕 LOUISIANA MACARONI KID SCRAPER\n' + '='.repeat(60));
  const sitesToScrape = fullMode ? LA_MK_SITES : LA_MK_SITES.slice(0, 5);
  ...
  console.log('\n' + '='.repeat(60));
  console.log('✅ LOUISIANA MACARONI KID SCRAPER COMPLETE!');
  console.log(`📊 Summary:`);
  ...
  await logScraperResult('Macaroni Kid LA', {
    new: imported,
    duplicates: skipped,
    errors: failed
  }, { state: 'LA', source: 'macaroni-kid' });

  return { imported, failed };
}

if (require.main === module) {
  scrapeMacaroniKidLouisiana()
    .then(() => process.exit(0))
    .catch(error => {
...
module.exports = { scrapeMacaroniKidLouisiana };
