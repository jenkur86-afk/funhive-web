...
/**
 * VIRGINIA MACARONI KID SCRAPER
 *
 * Scrapes events from 14 Virginia Macaroni Kid sites
 *
 * Coverage: Arlington-Alexandria, Fairfax-Burke-Oakton-Chantilly, Fredericksburg-Spotsylvania-Stafford, Gainesville, Lynchburg, McLean-Vienna-Falls Church, New River Valley, Newington, Norfolk, Reston-Herndon-Great Falls, Richmond, Roanoke, Virginia Beach, Williamsburg-Hampton
 *
 * Usage:
 *   node scraper-macaroni-va.js          # Test mode (5 sites)
 *   node scraper-macaroni-va.js --full   # Production (all 14 sites)
 */
...
// All 14 Virginia Macaroni Kid Sites
const VA_MK_SITES = [
  { url: 'https://arlington.macaronikid.com', name: 'Arlington-Alexandria', county: 'Arlington' },
  ...
  try {
    ...
    return await page.evaluate(() => {
      ...
      if (eventsIndex > -1) {
        ...
        if (idx < lines.length) { const cityZipMatch = lines[idx].match(/^([\w\s]+)\s+VA\s+(\d{5})/); if (cityZipMatch) { result.city = cityZipMatch[1].trim(); result.zipCode = cityZipMatch[2]; }}
      }
  ...
  try {
    ...
    for (const url of eventUrls) {
      ...
      if (coords) {
        ...
        if (!coords) {
          const countyCentroid = getCountyCentroid(site.county, 'VA');
          if (countyCentroid) {
      ...
      const eventDoc = {
        ...
        metadata: {
          source: 'Macaroni Kid Virginia Scraper',
...
async function scrapeMacaroniKidVirginia() {
  const fullMode = true; // PRODUCTION: All sites
  console.log('🥕 VIRGINIA MACARONI KID SCRAPER\n' + '='.repeat(60));
  const sitesToScrape = fullMode ? VA_MK_SITES : VA_MK_SITES.slice(0, 5);
  const maxEventsPerSite = fullMode ? 999 : 30; // No limit - filtered by 60 day date range
  console.log(`Mode: ${fullMode ? 'FULL (all 14 sites)' : 'LIMITED (5 sites)'}\nDate range: Next 60 days\n`);
  ...
  console.log('\n' + '='.repeat(60));
  console.log('✅ VIRGINIA MACARONI KID SCRAPER COMPLETE!');
  console.log(`📊 Summary:`);
  ...
  await logScraperResult('Macaroni Kid VA', {
    new: imported,
    duplicates: skipped,
    errors: failed
  }, { state: 'VA', source: 'macaroni-kid' });

  return { imported, failed };
}

if (require.main === module) {
  scrapeMacaroniKidVirginia()
    .then(() => process.exit(0))
    .catch(error => {
...
module.exports = { scrapeMacaroniKidVirginia };
