...
async function geocodeAddress(address, city, zipCode) {
  return _sharedGeocodeAddress(address, city, 'WV', zipCode);
}

async function geocodeVenue(venue, city, zipCode) {
  return _sharedGeocodeVenue(venue, city, 'WV', zipCode);
}
    ...
    for (const url of eventUrls) {
      ...
      if (coords) {
        ...
        if (!coords && details.city) {
          coords = await getCityCenterCoords(details.city, 'WV', details.zipCode);
          if (coords) {
            locationObj = {
              address: details.address || '',
              city: details.city,
      ...
      const eventDoc = {
        ...
        metadata: {
          ...
          sourceUrl: site.url,
          scraperName: `MacaroniKid-WV-${site.url.replace(/^https?:\/\//, '').replace(/^www\./, '').split('.')[0]}`,
