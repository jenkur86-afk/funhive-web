...
/**
 * WEST VIRGINIA MACARONI KID SCRAPER
 *
 * Scrapes events from 1 West Virginia Macaroni Kid sites
 *
 * Coverage: Parkersburg-North Hills-Marietta
 *
 * Usage:
 *   node scraper-macaroni-wv.js          # Test mode (5 sites)
 *   node scraper-macaroni-wv.js --full   # Production (all 1 sites)
 */
...
// All 1 West Virginia Macaroni Kid Sites
const WV_MK_SITES = [
  { url: 'https://parkersburg.macaronikid.com', name: 'Parkersburg-North Hills-Marietta', county: 'Wood' }
];

async function geocodeAddress(address, city, zipCode) {
  const fullAddress = `${address}, ${city}, WV ${zipCode}`;
  let result = await tryGeocode(fullAddress);
  if (result) return result;

  const cleaned = address.replace(/,?\s*Suite\s+[A-Z0-9-]+/i, '').replace(/,?\s*#\s*[A-Z0-9-]+/i, '');
  if (cleaned !== address) {
    result = await tryGeocode(`${cleaned}, ${city}, WV ${zipCode}`);
    if (result) return result;
  }

  const streetOnly = cleaned.split(',')[0];
  return await tryGeocode(`${streetOnly}, WV ${zipCode}`);
}
  ...
  try {
    ...
    return await page.evaluate(() => {
      ...
      if (eventsIndex > -1) {
        ...
        if (idx < lines.length) { const cityZipMatch = lines[idx].match(/^([\w\s]+)\s+WV\s+(\d{5})/); if (cityZipMatch) { result.city = cityZipMatch[1].trim(); result.zipCode = cityZipMatch[2]; }}
      }
  ...
  try {
    ...
    for (const url of eventUrls) {
      ...
      if (coords) {
        ...
        if (!coords) {
          const countyCentroid = getCountyCentroid(site.county, 'WV');
          if (countyCentroid) {
      ...
      const eventDoc = {
        ...
        metadata: {
          source: 'Macaroni Kid West Virginia Scraper',
...
async function scrapeMacaroniKidWestVirginia() {
  const fullMode = true;
  console.log('🥕 WEST VIRGINIA MACARONI KID SCRAPER\n' + '='.repeat(60));
  const sitesToScrape = fullMode ? WV_MK_SITES : WV_MK_SITES.slice(0, 5);
  ...
  console.log('\n' + '='.repeat(60));
  console.log('✅ WEST VIRGINIA MACARONI KID SCRAPER COMPLETE!');
  console.log(`📊 Summary:`);
  ...
  await logScraperResult('Macaroni Kid WV', {
    new: imported,
    duplicates: skipped,
    errors: failed
  }, { state: 'WV', source: 'macaroni-kid' });

  return { imported, failed };
}

if (require.main === module) {
  scrapeMacaroniKidWestVirginia()
    .then(() => process.exit(0))
    .catch(error => {
...
module.exports = { scrapeMacaroniKidWestVirginia };
