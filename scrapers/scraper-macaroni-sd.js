...
/**
 * SOUTH DAKOTA MACARONI KID SCRAPER
 *
 * Scrapes events from 1 South Dakota Macaroni Kid sites
 *
 * Coverage: Rapid City
 *
 * Usage:
 *   node scraper-macaroni-sd.js          # Test mode (5 sites)
 *   node scraper-macaroni-sd.js --full   # Production (all 1 sites)
 */
...
// All 1 South Dakota Macaroni Kid Sites
const SD_MK_SITES = [
  { url: 'https://rapidcity.macaronikid.com', name: 'Rapid City', county: 'Pennington' }
];

async function geocodeAddress(address, city, zipCode) {
  const fullAddress = `${address}, ${city}, SD ${zipCode}`;
  let result = await tryGeocode(fullAddress);
  if (result) return result;

  const cleaned = address.replace(/,?\s*Suite\s+[A-Z0-9-]+/i, '').replace(/,?\s*#\s*[A-Z0-9-]+/i, '');
  if (cleaned !== address) {
    result = await tryGeocode(`${cleaned}, ${city}, SD ${zipCode}`);
    if (result) return result;
  }

  const streetOnly = cleaned.split(',')[0];
  return await tryGeocode(`${streetOnly}, SD ${zipCode}`);
}
  ...
  try {
    ...
    return await page.evaluate(() => {
      ...
      if (eventsIndex > -1) {
        ...
        if (idx < lines.length) { const cityZipMatch = lines[idx].match(/^([\w\s]+)\s+SD\s+(\d{5})/); if (cityZipMatch) { result.city = cityZipMatch[1].trim(); result.zipCode = cityZipMatch[2]; }}
      }
  ...
  try {
    ...
    for (const url of eventUrls) {
      ...
      if (coords) {
        ...
        if (!coords) {
          const countyCentroid = getCountyCentroid(site.county, 'SD');
          if (countyCentroid) {
      ...
      const eventDoc = {
        ...
        metadata: {
          source: 'Macaroni Kid South Dakota Scraper',
...
async function scrapeMacaroniKidSouthDakota() {
  const fullMode = true;
  console.log('🥕 SOUTH DAKOTA MACARONI KID SCRAPER\n' + '='.repeat(60));
  const sitesToScrape = fullMode ? SD_MK_SITES : SD_MK_SITES.slice(0, 5);
  ...
  console.log('\n' + '='.repeat(60));
  console.log('✅ SOUTH DAKOTA MACARONI KID SCRAPER COMPLETE!');
  console.log(`📊 Summary:`);
  ...
  await logScraperResult('Macaroni Kid SD', {
    new: imported,
    duplicates: skipped,
    errors: failed
...
module.exports = { scrapeMacaroniKidSouthDakota };
