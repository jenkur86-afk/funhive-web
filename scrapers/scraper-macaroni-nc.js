...
/**
 * NORTH CAROLINA MACARONI KID SCRAPER
 *
 * Scrapes events from 14 North Carolina Macaroni Kid sites
 *
 * Coverage: Asheville, Burlington-Hillsborough, Cary, Chapel Hill-Durham, Durham, Fayetteville-Hope Mills-Fort Bragg, Garner-Clayton-Smithfield, Hendersonville, Hickory-Western Piedmont, Leland, South Charlotte, Union, Wake Forest, Waynesville-Bryson-Canton-Sylva
 *
 * Usage:
 *   node scraper-macaroni-nc.js          # Test mode (5 sites)
 *   node scraper-macaroni-nc.js --full   # Production (all 14 sites)
 */
  ...
  try {
    ...
    return await page.evaluate(() => {
      ...
      if (eventsIndex > -1) {
        ...
        if (idx < lines.length) { const cityZipMatch = lines[idx].match(/^([\w\s]+)\s+NC\s+(\d{5})/); if (cityZipMatch) { result.city = cityZipMatch[1].trim(); result.zipCode = cityZipMatch[2]; }}
      }
  ...
  try {
    ...
    for (const url of eventUrls) {
      ...
      const eventDoc = {
        ...
        metadata: {
          source: 'Macaroni Kid North Carolina Scraper',
...
async function scrapeMacaroniKidNorthCarolina() {
  ...
  console.log('🥕 NORTH CAROLINA MACARONI KID SCRAPER\n' + '='.repeat(60));
  const sitesToScrape = fullMode ? NC_MK_SITES : NC_MK_SITES.slice(0, 5);
  ...
  await logScraperResult('Macaroni Kid NC', {
    new: imported,
    duplicates: skipped,
    errors: failed
  }, { state: 'NC', source: 'macaroni-kid' });
