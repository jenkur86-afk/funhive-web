...
/**
 * KANSAS MACARONI KID SCRAPER
 *
 * Scrapes events from 3 Kansas Macaroni Kid sites
 *
 * Coverage: Gardner-Spring Hill-Paola, Kansas City-Leavenworth, Overland Park-Olathe-Shawnee
 *
 * Usage:
 *   node scraper-macaroni-ks.js          # Test mode (5 sites)
 *   node scraper-macaroni-ks.js --full   # Production (all 3 sites)
 */
...
// All 3 Kansas Macaroni Kid Sites
const KS_MK_SITES = [
  { url: 'https://paola.macaronikid.com', name: 'Gardner-Spring Hill-Paola', county: 'Miami' },
  { url: 'https://kansascityks.macaronikid.com', name: 'Kansas City-Leavenworth', county: 'Wyandotte' },
  { url: 'https://overlandpark.macaronikid.com', name: 'Overland Park-Olathe-Shawnee', county: 'Johnson' }
];

async function geocodeAddress(address, city, zipCode) {
  const fullAddress = `${address}, ${city}, KS ${zipCode}`;
  let result = await tryGeocode(fullAddress);
  if (result) return result;

  const cleaned = address.replace(/,?\s*Suite\s+[A-Z0-9-]+/i, '').replace(/,?\s*#\s*[A-Z0-9-]+/i, '');
  if (cleaned !== address) {
    result = await tryGeocode(`${cleaned}, ${city}, KS ${zipCode}`);
    if (result) return result;
  }

  const streetOnly = cleaned.split(',')[0];
  return await tryGeocode(`${streetOnly}, KS ${zipCode}`);
}
  ...
  try {
    ...
    return await page.evaluate(() => {
      ...
      if (eventsIndex > -1) {
        ...
        if (idx < lines.length) { const cityZipMatch = lines[idx].match(/^([\w\s]+)\s+KS\s+(\d{5})/); if (cityZipMatch) { result.city = cityZipMatch[1].trim(); result.zipCode = cityZipMatch[2]; }}
      }
  ...
  try {
    ...
    for (const url of eventUrls) {
      ...
      if (coords) {
        ...
        if (!coords) {
          const countyCentroid = getCountyCentroid(site.county, 'KS');
          if (countyCentroid) {
      ...
      const eventDoc = {
        ...
        metadata: {
          source: 'Macaroni Kid Kansas Scraper',
...
async function scrapeMacaroniKidKansas() {
  const fullMode = true;
  console.log('🥕 KANSAS MACARONI KID SCRAPER\n' + '='.repeat(60));
  const sitesToScrape = fullMode ? KS_MK_SITES : KS_MK_SITES.slice(0, 5);
  ...
  console.log('\n' + '='.repeat(60));
  console.log('✅ KANSAS MACARONI KID SCRAPER COMPLETE!');
  console.log(`📊 Summary:`);
  ...
  await logScraperResult('Macaroni Kid KS', {
    new: imported,
    duplicates: skipped,
    errors: failed
  }, { state: 'KS', source: 'macaroni-kid' });

  return { imported, failed };
}

if (require.main === module) {
  scrapeMacaroniKidKansas()
    .then(() => process.exit(0))
    .catch(error => {
...
module.exports = { scrapeMacaroniKidKansas };
