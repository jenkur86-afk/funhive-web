...
/**
 * ALABAMA MACARONI KID SCRAPER
 *
 * Scrapes events from 11 Alabama Macaroni Kid sites
 *
 * Coverage: Baldwin, Calhoun, Florence-Muscle Shoals, Marshall County, Mobile,
 *           Montgomery, North Birmingham, Owens Cross Roads-Scottsboro,
 *           South Birmingham, Tuscaloosa, Wiregrass
 *
 * Usage:
 *   node scraper-macaroni-al.js          # Test mode (5 sites)
 *   node scraper-macaroni-al.js --full   # Production (all 11 sites)
 */
...
// All 10 Alabama Macaroni Kid Sites (Marshall County retired)
const ALABAMA_MK_SITES = [
  { url: 'https://baldwin.macaronikid.com', name: 'Baldwin', county: 'Baldwin' },
  { url: 'https://calhounal.macaronikid.com', name: 'Calhoun', county: 'Calhoun' },
  { url: 'https://shoals.macaronikid.com', name: 'Florence-Muscle Shoals', county: 'Lauderdale' },
  // REMOVED: Marshall County — redirects to national.macaronikid.com (site retired)
  // { url: 'https://marshallcoal.macaronikid.com', name: 'Marshall County', county: 'Marshall' },
  { url: 'https://mobile.macaronikid.com', name: 'Mobile', county: 'Mobile' },
  { url: 'https://montgomeryal.macaronikid.com', name: 'Montgomery', county: 'Montgomery' },
  { url: 'https://northbirmingham.macaronikid.com', name: 'North Birmingham', county: 'Jefferson' },
  { url: 'https://scottsboro.macaronikid.com', name: 'Owens Cross Roads-Scottsboro', county: 'Jackson' },
  { url: 'https://sbirmingham.macaronikid.com', name: 'South Birmingham', county: 'Jefferson' },
  { url: 'https://tuscaloosa.macaronikid.com', name: 'Tuscaloosa', county: 'Tuscaloosa' },
  { url: 'https://ozark.macaronikid.com', name: 'Wiregrass', county: 'Dale' }
];

async function geocodeAddress(address, city, zipCode) {
  return _sharedGeocodeAddress(address, city, 'AL', zipCode);
}

async function geocodeVenue(venue, city, zipCode) {
  return _sharedGeocodeVenue(venue, city, 'AL', zipCode);
}
  ...
  try {
    ...
    for (const url of eventUrls) {
      ...
      if (coords) {
        ...
        if (details.city) {
          coords = await getCityCenterCoords(details.city, 'AL', details.zipCode);
          if (coords) {
            locationObj = {
              address: details.address || '',
              city: details.city,
        ...
        if (!coords) {
        const countyCentroid = getCountyCentroid(site.county, 'AL');
        if (countyCentroid) {
      ...
      const eventDoc = {
        ...
        metadata: {
          source: 'Macaroni Kid Alabama Scraper',
          sourceUrl: site.url,
          scraperName: `MacaroniKid-AL-${site.url.replace(/^https?:\/\//, '').replace(/^www\./, '').split('.')[0]}`,
...
async function scrapeMacaroniKidAlabama() {
  const fullMode = true; // PRODUCTION: All sites // Use limited mode for Cloud Functions
  console.log('🥕 ALABAMA MACARONI KID SCRAPER\n' + '='.repeat(60));
  const sitesToScrape = fullMode ? ALABAMA_MK_SITES : ALABAMA_MK_SITES.slice(0, 5);
  ...
  console.log('\n' + '='.repeat(60));
  console.log('✅ ALABAMA MACARONI KID SCRAPER COMPLETE!');
  console.log(`📊 Summary:`);
  ...
  await logScraperResult('Macaroni Kid AL', {
    new: imported,
    duplicates: skipped,
    errors: failed
  }, { state: 'AL', source: 'macaroni-kid' });
...
module.exports = { scrapeMacaroniKidAlabama };
