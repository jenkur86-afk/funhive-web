...
/**
 * NEW YORK MACARONI KID SCRAPER
 *
 * Scrapes events from 24 New York Macaroni Kid sites
 *
 * Coverage: Bayside-Northeast Queens, Bellmore-Merrick-Wantagh, Binghamton, Brooklyn NW, Central Westchester, Five Towns-Valley Stream-The Rockaways, Forest Hills-Corona-Rego Park, Greece-NW Rochester, Hamptons, Harlem-Washington Heights, Holbrook-The Ronkonkomas-Centereach-Coram, Hopewell-Wappingers-Fishkill, Long Beach-Oceanside-Rockville Centre, Lower Manhattan-Downtown, NYC Upper East Side, Pittsford-SE Rochester, Plattsburgh, Poughkeepsie-Pleasant Valley-Dover Plains, Riverdale-Kingsbridge-Inwood, Riverhead, South-Central Bronx, Southern Westchester, Upper West Side, Westchester Rivertowns
 *
 * Usage:
 *   node scraper-macaroni-ny.js          # Test mode (5 sites)
 *   node scraper-macaroni-ny.js --full   # Production (all 24 sites)
 */
...
// All 24 New York Macaroni Kid Sites
const NY_MK_SITES = [
  { url: 'https://bayside.macaronikid.com', name: 'Bayside-Northeast Queens', county: 'Queens' },
  { url: 'https://bellmore.macaronikid.com', name: 'Bellmore-Merrick-Wantagh', county: 'Nassau' },
  { url: 'https://binghamton.macaronikid.com', name: 'Binghamton', county: 'Broome' },
  { url: 'https://brooklynnw.macaronikid.com', name: 'Brooklyn NW', county: 'Kings' },
  { url: 'https://whiteplains.macaronikid.com', name: 'Central Westchester', county: 'Westchester' },
  { url: 'https://fivetowns.macaronikid.com', name: 'Five Towns-Valley Stream-The Rockaways', county: 'Nassau' },
  { url: 'https://foresthills.macaronikid.com', name: 'Forest Hills-Corona-Rego Park', county: 'Queens' },
  { url: 'https://greece.macaronikid.com', name: 'Greece-NW Rochester', county: 'Monroe' },
  { url: 'https://hamptons.macaronikid.com', name: 'Hamptons', county: 'Suffolk' },
  { url: 'https://harlem.macaronikid.com', name: 'Harlem-Washington Heights', county: 'New York' },
  { url: 'https://holbrook.macaronikid.com', name: 'Holbrook-The Ronkonkomas-Centereach-Coram', county: 'Suffolk' },
  { url: 'https://hopewelljunction.macaronikid.com', name: 'Hopewell-Wappingers-Fishkill', county: 'Dutchess' },
  { url: 'https://longbeach.macaronikid.com', name: 'Long Beach-Oceanside-Rockville Centre', county: 'Nassau' },
  { url: 'https://lowermanhattan.macaronikid.com', name: 'Lower Manhattan-Downtown', county: 'New York' },
  { url: 'https://nyceast.macaronikid.com', name: 'NYC Upper East Side', county: 'New York' },
  { url: 'https://pittsford.macaronikid.com', name: 'Pittsford-SE Rochester', county: 'Monroe' },
  { url: 'https://plattsburgh.macaronikid.com', name: 'Plattsburgh', county: 'Clinton' },
  { url: 'https://poughkeepsie.macaronikid.com', name: 'Poughkeepsie-Pleasant Valley-Dover Plains', county: 'Dutchess' },
  { url: 'https://riverdale.macaronikid.com', name: 'Riverdale-Kingsbridge-Inwood', county: 'Bronx' },
  { url: 'https://riverhead.macaronikid.com', name: 'Riverhead', county: 'Suffolk' },
  { url: 'https://southbronx.macaronikid.com', name: 'South-Central Bronx', county: 'Bronx' },
  { url: 'https://southernwestchester.macaronikid.com', name: 'Southern Westchester', county: 'Westchester' },
  { url: 'https://upperwestside.macaronikid.com', name: 'Upper West Side', county: 'New York' },
  { url: 'https://westchester-rivertowns.macaronikid.com', name: 'Westchester Rivertowns', county: 'Westchester' }
];

async function geocodeAddress(address, city, zipCode) {
  const fullAddress = `${address}, ${city}, NY ${zipCode}`;
  let result = await tryGeocode(fullAddress);
  if (result) return result;

  const cleaned = address.replace(/,?\s*Suite\s+[A-Z0-9-]+/i, '').replace(/,?\s*#\s*[A-Z0-9-]+/i, '');
  if (cleaned !== address) {
    result = await tryGeocode(`${cleaned}, ${city}, NY ${zipCode}`);
    if (result) return result;
  }

  const streetOnly = cleaned.split(',')[0];
  return await tryGeocode(`${streetOnly}, NY ${zipCode}`);
}
  ...
  try {
    ...
    return await page.evaluate(() => {
      ...
      if (eventsIndex > -1) {
        ...
        if (idx < lines.length) { const cityZipMatch = lines[idx].match(/^([\w\s]+)\s+NY\s+(\d{5})/); if (cityZipMatch) { result.city = cityZipMatch[1].trim(); result.zipCode = cityZipMatch[2]; }}
      }
  ...
  try {
    ...
    for (const url of eventUrls) {
      ...
      if (coords) {
        ...
        if (!coords) {
          const countyCentroid = getCountyCentroid(site.county, 'NY');
          if (countyCentroid) {
      ...
      const eventDoc = {
        ...
        metadata: {
          source: 'Macaroni Kid New York Scraper',
...
async function scrapeMacaroniKidNewYork() {
  const fullMode = true; // PRODUCTION: All sites
  console.log('🥕 NEW YORK MACARONI KID SCRAPER\n' + '='.repeat(60));
  const sitesToScrape = fullMode ? NY_MK_SITES : NY_MK_SITES.slice(0, 5);
  const maxEventsPerSite = fullMode ? 999 : 30; // No limit - filtered by 60 day date range
  console.log(`Mode: ${fullMode ? 'FULL (all 24 sites)' : 'LIMITED (5 sites)'}\nDate range: Next 60 days\n`);
  ...
  console.log('\n' + '='.repeat(60));
  console.log('✅ NEW YORK MACARONI KID SCRAPER COMPLETE!');
  console.log(`📊 Summary:`);
  ...
  await logScraperResult('Macaroni Kid NY', {
    new: imported,
    duplicates: skipped,
    errors: failed
  }, { state: 'NY', source: 'macaroni-kid' });

  return { imported, failed };
}

if (require.main === module) {
  scrapeMacaroniKidNewYork()
    .then(() => process.exit(0))
    .catch(error => {
...
module.exports = { scrapeMacaroniKidNewYork };
