...
const LIBRARIES = [
  ...
  { name: 'Laurel County Public Library', url: 'https://www.laurellibrary.org', eventsUrl: 'https://www.laurellibrary.org/events', city: 'London', state: 'KY', zipCode: '40741', county: 'Laurel', urlCollision: 'publishes on laurellibrary.bibliocommons.com - link taken from the library own page, KY confirmed by ZIP 40741 London and area code 606. Relocated to BiblioCommons-KY on 2026-08-27'},
  ...
  // 2026-09-03: WRONG-STATE URL, a new instance of Defect A, found via the age audit rather than gate 2 - the flagged-site table listed a venue reading "Louis T. Graves Memorial Public Library @ 18 Maine St, Kennebunkport, ME 04046, USA" under scraper WordPress-KY. graveslibrary.org is that MAINE library, ph 207-967-2778, proven from its own page title and contact block via the stealth browser since it serves Cloudflare 403 to a plain fetch. It was importing Maine events stored as state KY, city Louisville. The real library is Graves County Public Library, 601 N 17th St, Mayfield KY 42066, ph 270-247-2911 - ZIP matches this entry - and it runs LibraryCalendar, so it was relocated to LibraryCalendar-Libraries.
  ...
  { name: 'Calloway County Public Library', url: 'https://www.callowaycountylibrary.org', eventsUrl: 'https://www.callowaycountylibrary.org/events', city: 'Murray', state: 'KY', zipCode: '42071' , urlCollision: 'relocated to LibraryCalendar-Libraries 2026-09-07 - this library publishes on callowaycounty.librarycalendar.com, a LibraryCalendar instance that a WordPress DOM extractor cannot read. Host was discovered in this library own page markup and then confirmed live: the page titles itself Calloway County Public Library and its programme renders server-side. The WordPress entry is guarded rather than deleted so the library keeps its audit row'},
  { name: 'Woodford County Library', url: 'https://www.woodfordlibrary.org', eventsUrl: 'https://www.woodfordlibrary.org/events', city: 'Versailles', state: 'KY', zipCode: '40383', county: 'Woodford', urlCollision: "host is DEAD, probed 2026-09-07: the origin answered nothing on any of 8 paths (/, /events, /events/, /calendar, /calendar/, /events/upcoming, /whats-on, /programs). This is a host failure, not a wrong path - the distinction was checked rather than assumed, because a bare 404 on one path is not evidence a site is gone. Guarded so the rotation stops visiting it. OPEN COVERAGE GAP until a real URL is found" },
  ...
  { name: 'Goodnight Memorial Library', url: 'https://www.franklinlibrary.org', eventsUrl: 'https://www.franklinlibrary.org/events', city: 'Franklin', state: 'KY', zipCode: '42134', county: 'Franklin County', urlCollision: 'franklinlibrary.org is dead or serves an unrelated site — no state entry is correct' },
  ...
  { name: 'Harlan County Public Library', url: 'https://www.harlanlibrary.org', eventsUrl: 'https://www.harlanlibrary.org/events', city: 'Harlan', state: 'KY', zipCode: '40831', county: 'Harlan County', urlCollision: "dead/hijacked domain, confirmed: dead-domain: the configured URL redirects to harlan.lib.ia.us, which serves IA, not KY — a same-named town in another state, the classic guessed-{city}library.org failure; needs a correct URL or a guard Guarded 2026-09-07 so the rotation stops visiting it — a squatted domain is one selector change away from importing another site's markup as events. NOT deleted: this stays an explained OPEN COVERAGE GAP until a real URL is found." },
