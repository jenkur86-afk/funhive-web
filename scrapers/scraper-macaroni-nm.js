...
/**
 * NEW MEXICO MACARONI KID SCRAPER
 *
 * Scrapes events from 1 New Mexico Macaroni Kid sites
 *
 * Coverage: Santa Fe
 *
 * Usage:
 *   node scraper-macaroni-nm.js          # Test mode (5 sites)
 *   node scraper-macaroni-nm.js --full   # Production (all 1 sites)
 */
...
// All 1 New Mexico Macaroni Kid Sites
const NM_MK_SITES = [
  { url: 'https://santafe.macaronikid.com', name: 'Santa Fe', county: 'Santa Fe' }
];

async function geocodeAddress(address, city, zipCode) {
  const fullAddress = `${address}, ${city}, NM ${zipCode}`;
  let result = await tryGeocode(fullAddress);
  if (result) return result;

  const cleaned = address.replace(/,?\s*Suite\s+[A-Z0-9-]+/i, '').replace(/,?\s*#\s*[A-Z0-9-]+/i, '');
  if (cleaned !== address) {
    result = await tryGeocode(`${cleaned}, ${city}, NM ${zipCode}`);
    if (result) return result;
  }

  const streetOnly = cleaned.split(',')[0];
  return await tryGeocode(`${streetOnly}, NM ${zipCode}`);
}
  ...
  try {
    ...
    return await page.evaluate(() => {
      ...
      if (eventsIndex > -1) {
        ...
        if (idx < lines.length) { const cityZipMatch = lines[idx].match(/^([\w\s]+)\s+NM\s+(\d{5})/); if (cityZipMatch) { result.city = cityZipMatch[1].trim(); result.zipCode = cityZipMatch[2]; }}
      }
  ...
  try {
    ...
    for (const url of eventUrls) {
      ...
      if (coords) {
        ...
        if (!coords) {
          const countyCentroid = getCountyCentroid(site.county, 'NM');
          if (countyCentroid) {
      ...
      const eventDoc = {
        ...
        metadata: {
          source: 'Macaroni Kid New Mexico Scraper',
...
async function scrapeMacaroniKidNewMexico() {
  const fullMode = true;
  console.log('🥕 NEW MEXICO MACARONI KID SCRAPER\n' + '='.repeat(60));
  const sitesToScrape = fullMode ? NM_MK_SITES : NM_MK_SITES.slice(0, 5);
  ...
  console.log('\n' + '='.repeat(60));
  console.log('✅ NEW MEXICO MACARONI KID SCRAPER COMPLETE!');
  console.log(`📊 Summary:`);
  ...
  await logScraperResult('Macaroni Kid NM', {
    new: imported,
    duplicates: skipped,
    errors: failed
  }, { state: 'NM', source: 'macaroni-kid' });

  return { imported, failed };
}

if (require.main === module) {
  scrapeMacaroniKidNewMexico()
    .then(() => process.exit(0))
    .catch(error => {
...
module.exports = { scrapeMacaroniKidNewMexico };
