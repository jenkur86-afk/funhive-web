...
async function geocodeAddress(address, city, zipCode) {
  return _sharedGeocodeAddress(address, city, 'NM', zipCode);
}

async function geocodeVenue(venue, city, zipCode) {
  return _sharedGeocodeVenue(venue, city, 'NM', zipCode);
}
    ...
    for (const url of eventUrls) {
      ...
      if (coords) {
        ...
        if (!coords && details.city) {
          coords = await getCityCenterCoords(details.city, 'NM', details.zipCode);
          if (coords) {
            locationObj = {
              address: details.address || '',
              city: details.city,
      ...
      const eventDoc = {
        ...
        metadata: {
          ...
          sourceUrl: site.url,
          scraperName: `MacaroniKid-NM-${site.url.replace(/^https?:\/\//, '').replace(/^www\./, '').split('.')[0]}`,
