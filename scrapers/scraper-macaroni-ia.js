...
/**
 * IOWA MACARONI KID SCRAPER
 *
 * Scrapes events from 2 Iowa Macaroni Kid sites
 *
 * Coverage: Cedar Falls-Waterloo, Cedar Rapids-Iowa City
 *
 * Usage:
 *   node scraper-macaroni-ia.js          # Test mode (5 sites)
 *   node scraper-macaroni-ia.js --full   # Production (all 2 sites)
 */
...
const IA_MK_SITES = [
  { url: 'https://cedarfalls.macaronikid.com', name: 'Cedar Falls-Waterloo', county: 'Black Hawk' },
  { url: 'https://cedarrapids.macaronikid.com', name: 'Cedar Rapids-Iowa City', county: 'Linn' }
];

async function geocodeAddress(address, city, zipCode) {
  const fullAddress = `${address}, ${city}, IA ${zipCode}`;
  let result = await tryGeocode(fullAddress);
  if (result) return result;

  const cleaned = address.replace(/,?\s*Suite\s+[A-Z0-9-]+/i, '').replace(/,?\s*#\s*[A-Z0-9-]+/i, '');
  if (cleaned !== address) {
    result = await tryGeocode(`${cleaned}, ${city}, IA ${zipCode}`);
    if (result) return result;
  }

  const streetOnly = cleaned.split(',')[0];
  return await tryGeocode(`${streetOnly}, IA ${zipCode}`);
}
  ...
  try {
    ...
    return await page.evaluate(() => {
      ...
      if (eventsIndex > -1) {
        ...
        if (idx < lines.length) { const cityZipMatch = lines[idx].match(/^([\w\s]+)\s+IA\s+(\d{5})/); if (cityZipMatch) { result.city = cityZipMatch[1].trim(); result.zipCode = cityZipMatch[2]; }}
      }
  ...
  try {
    ...
    for (const url of eventUrls) {
      ...
      if (coords) {
        ...
        if (!coords) {
          const countyCentroid = getCountyCentroid(site.county, 'IA');
          if (countyCentroid) {
      ...
      const eventDoc = {
        ...
        metadata: {
          source: 'Macaroni Kid Iowa Scraper',
...
async function scrapeMacaroniKidIowa() {
  const fullMode = true;
  console.log('🥕 IOWA MACARONI KID SCRAPER\n' + '='.repeat(60));
  const sitesToScrape = fullMode ? IA_MK_SITES : IA_MK_SITES.slice(0, 5);
  ...
  console.log('\n' + '='.repeat(60));
  console.log('✅ IOWA MACARONI KID SCRAPER COMPLETE!');
  console.log(`📊 Summary:`);
  ...
  await logScraperResult('Macaroni Kid IA', {
    new: imported,
    duplicates: skipped,
    errors: failed
...
module.exports = { scrapeMacaroniKidIowa };
