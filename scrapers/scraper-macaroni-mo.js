...
/**
 * MISSOURI MACARONI KID SCRAPER
 *
 * Scrapes events from 5 Missouri Macaroni Kid sites
 *
 * Coverage: Columbia, Downtown-South Kansas City, Northland Kansas City, SE Kansas City, Springfield
 *
 * Usage:
 *   node scraper-macaroni-mo.js          # Test mode (5 sites)
 *   node scraper-macaroni-mo.js --full   # Production (all 5 sites)
 */
...
const MO_MK_SITES = [
  { url: 'https://columbiamo.macaronikid.com', name: 'Columbia', county: 'Boone' },
  { url: 'https://downtownkansascity.macaronikid.com', name: 'Downtown-South Kansas City', county: 'Jackson' },
  { url: 'https://northlandkansascity.macaronikid.com', name: 'Northland Kansas City', county: 'Clay' },
  { url: 'https://sekansascity.macaronikid.com', name: 'SE Kansas City', county: 'Jackson' },
  { url: 'https://springfieldmo.macaronikid.com', name: 'Springfield', county: 'Greene' }
];

async function geocodeAddress(address, city, zipCode) {
  const fullAddress = `${address}, ${city}, MO ${zipCode}`;
  let result = await tryGeocode(fullAddress);
  if (result) return result;

  const cleaned = address.replace(/,?\s*Suite\s+[A-Z0-9-]+/i, '').replace(/,?\s*#\s*[A-Z0-9-]+/i, '');
  if (cleaned !== address) {
    result = await tryGeocode(`${cleaned}, ${city}, MO ${zipCode}`);
    if (result) return result;
  }

  const streetOnly = cleaned.split(',')[0];
  return await tryGeocode(`${streetOnly}, MO ${zipCode}`);
}
  ...
  try {
    ...
    return await page.evaluate(() => {
      ...
      if (eventsIndex > -1) {
        ...
        if (idx < lines.length) { const cityZipMatch = lines[idx].match(/^([\w\s]+)\s+MO\s+(\d{5})/); if (cityZipMatch) { result.city = cityZipMatch[1].trim(); result.zipCode = cityZipMatch[2]; }}
      }
  ...
  try {
    ...
    for (const url of eventUrls) {
      ...
      if (coords) {
        ...
        if (!coords) {
          const countyCentroid = getCountyCentroid(site.county, 'MO');
          if (countyCentroid) {
      ...
      const eventDoc = {
        ...
        metadata: {
          source: 'Macaroni Kid Missouri Scraper',
...
async function scrapeMacaroniKidMissouri() {
  const fullMode = true;
  console.log('🥕 MISSOURI MACARONI KID SCRAPER\n' + '='.repeat(60));
  const sitesToScrape = fullMode ? MO_MK_SITES : MO_MK_SITES.slice(0, 5);
  ...
  console.log('\n' + '='.repeat(60));
  console.log('✅ MISSOURI MACARONI KID SCRAPER COMPLETE!');
  console.log(`📊 Summary:`);
  ...
  await logScraperResult('Macaroni Kid MO', {
    new: imported,
    duplicates: skipped,
    errors: failed
  }, { state: 'MO', source: 'macaroni-kid' });

  return { imported, failed };
}

if (require.main === module) {
  scrapeMacaroniKidMissouri()
    .then(() => process.exit(0))
    .catch(error => {
...
module.exports = { scrapeMacaroniKidMissouri };
