  ...
  try {
    ...
    return await page.evaluate(() => {
      ...
      if (eventsIndex > -1) {
        ...
        if (idx < lines.length && !/^\d/.test(lines[idx]) && lines[idx].length < 100) { result.venue = lines[idx]; idx++; }
        if (idx < lines.length && /^\d+\s+[\w\s]+/.test(lines[idx])) { result.address = lines[idx]; idx++; }
        if (idx < lines.length) { const cityZipMatch = lines[idx].match(/^([\w\s]+)\s+MD\s+(\d{5})/); if (cityZipMatch) { result.city = cityZipMatch[1].trim(); result.zipCode = cityZipMatch[2]; }}
      }
...
async function scrapeSite(browser, site, maxEvents = 50) {
  ...
  try {
    ...
    let imported = 0, skippedPast = 0, failedGeocode = 0, noLocation = 0;
