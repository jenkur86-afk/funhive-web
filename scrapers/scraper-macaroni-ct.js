...
/**
 * CONNECTICUT MACARONI KID SCRAPER
 *
 * Scrapes events from 9 Connecticut Macaroni Kid sites
 *
 * Coverage: Greater Danbury, Enfield, Fairfield-Trumbull-Shelton, Greenwich to Westport, Hartford, Old Lyme-Madison-Shoreline Towns, Southbury, Southington-Bristol-Cheshire, Wallingford-North Haven
 *
 * Usage:
 *   node scraper-macaroni-ct.js          # Test mode (5 sites)
 *   node scraper-macaroni-ct.js --full   # Production (all 9 sites)
 */
...
// All 9 Connecticut Macaroni Kid Sites
const CT_MK_SITES = [
  { url: 'https://danbury.macaronikid.com', name: 'Greater Danbury', county: 'Fairfield' },
  { url: 'https://enfield.macaronikid.com', name: 'Enfield', county: 'Hartford' },
  { url: 'https://monroe.macaronikid.com', name: 'Fairfield-Trumbull-Shelton', county: 'Fairfield' },
  { url: 'https://westport.macaronikid.com', name: 'Greenwich to Westport', county: 'Fairfield' },
  { url: 'https://hartford.macaronikid.com', name: 'Hartford', county: 'Hartford' },
  { url: 'https://madison.macaronikid.com', name: 'Old Lyme-Madison-Shoreline Towns', county: 'Middlesex' },
  { url: 'https://southbury.macaronikid.com', name: 'Southbury', county: 'New Haven' },
  { url: 'https://bristol.macaronikid.com', name: 'Southington-Bristol-Cheshire', county: 'Hartford' },
  { url: 'https://wallingford.macaronikid.com', name: 'Wallingford-North Haven', county: 'New Haven' }
];

async function geocodeAddress(address, city, zipCode) {
  const fullAddress = `${address}, ${city}, CT ${zipCode}`;
  let result = await tryGeocode(fullAddress);
  if (result) return result;

  const cleaned = address.replace(/,?\s*Suite\s+[A-Z0-9-]+/i, '').replace(/,?\s*#\s*[A-Z0-9-]+/i, '');
  if (cleaned !== address) {
    result = await tryGeocode(`${cleaned}, ${city}, CT ${zipCode}`);
    if (result) return result;
  }

  const streetOnly = cleaned.split(',')[0];
  return await tryGeocode(`${streetOnly}, CT ${zipCode}`);
}
  ...
  try {
    ...
    return await page.evaluate(() => {
      ...
      if (eventsIndex > -1) {
        ...
        if (idx < lines.length) { const cityZipMatch = lines[idx].match(/^([\w\s]+)\s+CT\s+(\d{5})/); if (cityZipMatch) { result.city = cityZipMatch[1].trim(); result.zipCode = cityZipMatch[2]; }}
      }
  ...
  try {
    ...
    for (const url of eventUrls) {
      ...
      if (coords) {
        ...
        if (!coords) {
          const countyCentroid = getCountyCentroid(site.county, 'CT');
          if (countyCentroid) {
      ...
      const eventDoc = {
        ...
        metadata: {
          source: 'Macaroni Kid Connecticut Scraper',
...
async function scrapeMacaroniKidConnecticut() {
  const fullMode = true; // PRODUCTION: All sites
  console.log('🥕 CONNECTICUT MACARONI KID SCRAPER\n' + '='.repeat(60));
  const sitesToScrape = fullMode ? CT_MK_SITES : CT_MK_SITES.slice(0, 5);
  ...
  console.log('\n' + '='.repeat(60));
  console.log('✅ CONNECTICUT MACARONI KID SCRAPER COMPLETE!');
  console.log(`📊 Summary:`);
  ...
  await logScraperResult('Macaroni Kid CT', {
    new: imported,
    duplicates: skipped,
    errors: failed
  }, { state: 'CT', source: 'macaroni-kid' });

  return { imported, failed };
}

if (require.main === module) {
  scrapeMacaroniKidConnecticut()
    .then(() => process.exit(0))
    .catch(error => {
...
module.exports = { scrapeMacaroniKidConnecticut };
