...
/**
 * MICHIGAN MACARONI KID SCRAPER
 *
 * Scrapes events from 6 Michigan Macaroni Kid sites
 *
 * Coverage: Ferndale-Royal Oak, Grand Rapids, Grand Haven-Spring Lake-Muskegon, Lake Orion-Rochester Hills-Oxford, Livingston, Macomb
 *
 * Usage:
 *   node scraper-macaroni-mi.js          # Test mode (5 sites)
 *   node scraper-macaroni-mi.js --full   # Production (all 6 sites)
 */
...
// All 6 Michigan Macaroni Kid Sites
const MI_MK_SITES = [
  { url: 'https://ferndale.macaronikid.com', name: 'Ferndale-Royal Oak', county: 'Oakland' },
  { url: 'https://grandrapids.macaronikid.com', name: 'Grand Rapids', county: 'Kent' },
  { url: 'https://grandhaven.macaronikid.com', name: 'Grand Haven-Spring Lake-Muskegon', county: 'Ottawa' },
  { url: 'https://lakeorion.macaronikid.com', name: 'Lake Orion-Rochester Hills-Oxford', county: 'Oakland' },
  { url: 'https://livingston.macaronikid.com', name: 'Livingston', county: 'Livingston' },
  { url: 'https://macomb.macaronikid.com', name: 'Macomb', county: 'Macomb' }
];

async function geocodeAddress(address, city, zipCode) {
  const fullAddress = `${address}, ${city}, MI ${zipCode}`;
  let result = await tryGeocode(fullAddress);
  if (result) return result;

  const cleaned = address.replace(/,?\s*Suite\s+[A-Z0-9-]+/i, '').replace(/,?\s*#\s*[A-Z0-9-]+/i, '');
  if (cleaned !== address) {
    result = await tryGeocode(`${cleaned}, ${city}, MI ${zipCode}`);
    if (result) return result;
  }

  const streetOnly = cleaned.split(',')[0];
  return await tryGeocode(`${streetOnly}, MI ${zipCode}`);
}
  ...
  try {
    ...
    return await page.evaluate(() => {
      ...
      if (eventsIndex > -1) {
        ...
        if (idx < lines.length) { const cityZipMatch = lines[idx].match(/^([\w\s]+)\s+MI\s+(\d{5})/); if (cityZipMatch) { result.city = cityZipMatch[1].trim(); result.zipCode = cityZipMatch[2]; }}
      }
  ...
  try {
    ...
    for (const url of eventUrls) {
      ...
      if (coords) {
        ...
        if (!coords) {
          const countyCentroid = getCountyCentroid(site.county, 'MI');
          if (countyCentroid) {
      ...
      const eventDoc = {
        ...
        metadata: {
          source: 'Macaroni Kid Michigan Scraper',
...
async function scrapeMacaroniKidMichigan() {
  const fullMode = true; // PRODUCTION: All sites
  console.log('🥕 MICHIGAN MACARONI KID SCRAPER\n' + '='.repeat(60));
  const sitesToScrape = fullMode ? MI_MK_SITES : MI_MK_SITES.slice(0, 5);
  const maxEventsPerSite = fullMode ? 999 : 30; // No limit - filtered by 60 day date range
  console.log(`Mode: ${fullMode ? 'FULL (all 6 sites)' : 'LIMITED (5 sites)'}\nDate range: Next 60 days\n`);
  ...
  console.log('\n' + '='.repeat(60));
  console.log('✅ MICHIGAN MACARONI KID SCRAPER COMPLETE!');
  console.log(`📊 Summary:`);
  ...
  await logScraperResult('Macaroni Kid MI', {
    new: imported,
    duplicates: skipped,
    errors: failed
  }, { state: 'MI', source: 'macaroni-kid' });

  return { imported, failed };
}

if (require.main === module) {
  scrapeMacaroniKidMichigan()
    .then(() => process.exit(0))
    .catch(error => {
...
module.exports = { scrapeMacaroniKidMichigan };
