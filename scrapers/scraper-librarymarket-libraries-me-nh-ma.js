...
const LIBRARIES = [
  ...
  },
  {
    ...
    eventsUrl: 'https://www.auburnpubliclibrary.org/events/upcoming',
    city: 'Auburn',
    state: 'ME',
    zipCode: '04210'
  },
...
const SCRAPER_NAME = 'librarymarket-ME-NH-MA';
...
async function main() {
  console.log(`\n╔════════════════════════════════════════════════════════╗`);
  console.log(`║  Generic Scraper - ME-NH-MA (${LIBRARIES.length} libraries)  ║`);
  console.log(`╚════════════════════════════════════════════════════════╝\n`);
  ...
  await logScraperResult('Librarymarket Libraries ME-NH-MA', {
  ...
  }, { state: 'ME-NH-MA', source: 'librarymarket' });
 ...
 */
async function scrapeLibraryMarketMENHMACloudFunction() {
  console.log('☁️ Running librarymarket-ME-NH-MA as Cloud Function');
  const events = await scrapeGenericEvents();
  if (events.length > 0) {
    await saveToDatabase(events);
  }
...
module.exports = { scrapeGenericEvents, saveToDatabase, scrapeLibraryMarketMENHMACloudFunction };
