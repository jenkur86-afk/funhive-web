...
/**
 * NEW HAMPSHIRE MACARONI KID SCRAPER
 *
 * Scrapes events from 5 New Hampshire Macaroni Kid sites
 *
 * Coverage: Concord, Derry-Hampstead-Salem-Pelham, Manchester-Bedford, Nashua-Merrimack, Seacoast
 *
 * Usage:
 *   node scraper-macaroni-nh.js          # Test mode (5 sites)
 *   node scraper-macaroni-nh.js --full   # Production (all 5 sites)
 */
...
// All 5 New Hampshire Macaroni Kid Sites
const NH_MK_SITES = [
  { url: 'https://concordnh.macaronikid.com', name: 'Concord', county: 'Merrimack' },
  { url: 'https://derry.macaronikid.com', name: 'Derry-Hampstead-Salem-Pelham', county: 'Rockingham' },
  { url: 'https://manchester.macaronikid.com', name: 'Manchester-Bedford', county: 'Hillsborough' },
  { url: 'https://nashua.macaronikid.com', name: 'Nashua-Merrimack', county: 'Hillsborough' },
  { url: 'https://seacoast.macaronikid.com', name: 'Seacoast', county: 'Rockingham' }
];

async function geocodeAddress(address, city, zipCode) {
  const fullAddress = `${address}, ${city}, NH ${zipCode}`;
  let result = await tryGeocode(fullAddress);
  if (result) return result;

  const cleaned = address.replace(/,?\s*Suite\s+[A-Z0-9-]+/i, '').replace(/,?\s*#\s*[A-Z0-9-]+/i, '');
  if (cleaned !== address) {
    result = await tryGeocode(`${cleaned}, ${city}, NH ${zipCode}`);
    if (result) return result;
  }

  const streetOnly = cleaned.split(',')[0];
  return await tryGeocode(`${streetOnly}, NH ${zipCode}`);
}
  ...
  try {
    ...
    return await page.evaluate(() => {
      ...
      if (eventsIndex > -1) {
        ...
        if (idx < lines.length) { const cityZipMatch = lines[idx].match(/^([\w\s]+)\s+NH\s+(\d{5})/); if (cityZipMatch) { result.city = cityZipMatch[1].trim(); result.zipCode = cityZipMatch[2]; }}
      }
  ...
  try {
    ...
    for (const url of eventUrls) {
      ...
      if (coords) {
        ...
        if (!coords) {
          const countyCentroid = getCountyCentroid(site.county, 'NH');
          if (countyCentroid) {
      ...
      const eventDoc = {
        ...
        metadata: {
          source: 'Macaroni Kid New Hampshire Scraper',
...
async function scrapeMacaroniKidNewHampshire() {
  const fullMode = true;
  console.log('🥕 NEW HAMPSHIRE MACARONI KID SCRAPER\n' + '='.repeat(60));
  const sitesToScrape = fullMode ? NH_MK_SITES : NH_MK_SITES.slice(0, 5);
  const maxEventsPerSite = fullMode ? 999 : 30; // No limit - filtered by 60 day date range
  console.log(`Mode: ${fullMode ? 'FULL (all 5 sites)' : 'LIMITED (5 sites)'}\nDate range: Next 60 days\n`);
  ...
  console.log('\n' + '='.repeat(60));
  console.log('✅ NEW HAMPSHIRE MACARONI KID SCRAPER COMPLETE!');
  console.log(`📊 Summary:`);
  ...
  await logScraperResult('Macaroni Kid NH', {
    new: imported,
    duplicates: skipped,
    errors: failed
  }, { state: 'NH', source: 'macaroni-kid' });

  return { imported, failed };
}

if (require.main === module) {
  scrapeMacaroniKidNewHampshire()
    .then(() => process.exit(0))
    .catch(error => {
...
module.exports = { scrapeMacaroniKidNewHampshire };
