...
async function geocodeAddress(address, city, zipCode) {
  return _sharedGeocodeAddress(address, city, 'NH', zipCode);
}

async function geocodeVenue(venue, city, zipCode) {
  return _sharedGeocodeVenue(venue, city, 'NH', zipCode);
}
    ...
    for (const url of eventUrls) {
      ...
      if (coords) {
        ...
        if (!coords && details.city) {
          coords = await getCityCenterCoords(details.city, 'NH', details.zipCode);
          if (coords) {
            locationObj = {
              address: details.address || '',
              city: details.city,
      ...
      const eventDoc = {
        ...
        metadata: {
          ...
          sourceUrl: site.url,
          scraperName: `MacaroniKid-NH-${site.url.replace(/^https?:\/\//, '').replace(/^www\./, '').split('.')[0]}`,
