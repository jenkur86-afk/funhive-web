...
/**
 * MASSACHUSETTS MACARONI KID SCRAPER
 *
 * Scrapes events from 27 Massachusetts Macaroni Kid sites
 *
 * Coverage: Acton-Concord-Bedford-Lexington, Amherst Northampton, Attleboro-Mansfield-Taunton, Auburn, Berkshires, Bridgewater-Brockton, Burlington-Wilmington-Woburn-Winchester, Cape Cod, Chicopee-Holyoke-Westfield, Framingham-Natick-Sudbury, Franklin-Milford-E Blackstone Valley, Leominster-Clinton, Lowell, Lynnfield-Melrose-Stoneham-Reading-Wakefield, Merrimack Valley, Needham-Wellesley-Dedham, Newton-Brookline, North Shore, Palmer-Monson-Ware, Peabody-Salem-Marblehead, Plymouth, SW Boston, South Shore Boston, Springfield, Waltham-Belmont-Watertown, Weymouth-Canton-Quincy, Worcester
 *
 * Usage:
 *   node scraper-macaroni-ma.js          # Test mode (5 sites)
 *   node scraper-macaroni-ma.js --full   # Production (all 27 sites)
 */
...
// All 27 Massachusetts Macaroni Kid Sites
const MA_MK_SITES = [
  { url: 'https://lexington.macaronikid.com', name: 'Acton-Concord-Bedford-Lexington', county: 'Middlesex' },
  { url: 'https://amherst.macaronikid.com', name: 'Amherst Northampton', county: 'Hampshire' },
  { url: 'https://attleboro.macaronikid.com', name: 'Attleboro-Mansfield-Taunton', county: 'Bristol' },
  { url: 'https://auburnma.macaronikid.com', name: 'Auburn', county: 'Worcester' },
  { url: 'https://berkshires.macaronikid.com', name: 'Berkshires', county: 'Berkshire' },
  { url: 'https://bridgewater.macaronikid.com', name: 'Bridgewater-Brockton', county: 'Plymouth' },
  { url: 'https://burlingtonma.macaronikid.com', name: 'Burlington-Wilmington-Woburn-Winchester', county: 'Middlesex' },
  { url: 'https://capecod.macaronikid.com', name: 'Cape Cod', county: 'Barnstable' },
  { url: 'https://chicopee.macaronikid.com', name: 'Chicopee-Holyoke-Westfield', county: 'Hampden' },
  { url: 'https://framingham.macaronikid.com', name: 'Framingham-Natick-Sudbury', county: 'Middlesex' },
  { url: 'https://franklin.macaronikid.com', name: 'Franklin-Milford-E Blackstone Valley', county: 'Norfolk' },
  { url: 'https://leominster.macaronikid.com', name: 'Leominster-Clinton', county: 'Worcester' },
  { url: 'https://lowell.macaronikid.com', name: 'Lowell', county: 'Middlesex' },
  { url: 'https://wakefield.macaronikid.com', name: 'Lynnfield-Melrose-Stoneham-Reading-Wakefield', county: 'Essex' },
  { url: 'https://merrimackvalleyma.macaronikid.com', name: 'Merrimack Valley', county: 'Essex' },
  { url: 'https://needham.macaronikid.com', name: 'Needham-Wellesley-Dedham', county: 'Norfolk' },
  { url: 'https://newton.macaronikid.com', name: 'Newton-Brookline', county: 'Middlesex' },
  { url: 'https://northshorema.macaronikid.com', name: 'North Shore', county: 'Essex' },
  { url: 'https://palmermonson.macaronikid.com', name: 'Palmer-Monson-Ware', county: 'Hampden' },
  { url: 'https://salempeabody.macaronikid.com', name: 'Peabody-Salem-Marblehead', county: 'Essex' },
  { url: 'https://plymouthma.macaronikid.com', name: 'Plymouth', county: 'Plymouth' },
  { url: 'https://swboston.macaronikid.com', name: 'SW Boston', county: 'Norfolk' },
  { url: 'https://ssboston.macaronikid.com', name: 'South Shore Boston', county: 'Plymouth' },
  { url: 'https://springfield.macaronikid.com', name: 'Springfield', county: 'Hampden' },
  { url: 'https://waltham.macaronikid.com', name: 'Waltham-Belmont-Watertown', county: 'Middlesex' },
  { url: 'https://weymouth.macaronikid.com', name: 'Weymouth-Canton-Quincy', county: 'Norfolk' },
  { url: 'https://northworcester.macaronikid.com', name: 'Worcester', county: 'Worcester' }
];

async function geocodeAddress(address, city, zipCode) {
  const fullAddress = `${address}, ${city}, MA ${zipCode}`;
  let result = await tryGeocode(fullAddress);
  if (result) return result;

  const cleaned = address.replace(/,?\s*Suite\s+[A-Z0-9-]+/i, '').replace(/,?\s*#\s*[A-Z0-9-]+/i, '');
  if (cleaned !== address) {
    result = await tryGeocode(`${cleaned}, ${city}, MA ${zipCode}`);
    if (result) return result;
  }

  const streetOnly = cleaned.split(',')[0];
  return await tryGeocode(`${streetOnly}, MA ${zipCode}`);
}
  ...
  try {
    ...
    return await page.evaluate(() => {
      ...
      if (eventsIndex > -1) {
        ...
        if (idx < lines.length) { const cityZipMatch = lines[idx].match(/^([\w\s]+)\s+MA\s+(\d{5})/); if (cityZipMatch) { result.city = cityZipMatch[1].trim(); result.zipCode = cityZipMatch[2]; }}
      }
  ...
  try {
    ...
    for (const url of eventUrls) {
      ...
      if (coords) {
        ...
        if (!coords) {
          const countyCentroid = getCountyCentroid(site.county, 'MA');
          if (countyCentroid) {
      ...
      const eventDoc = {
        ...
        metadata: {
          source: 'Macaroni Kid Massachusetts Scraper',
...
async function scrapeMacaroniKidMassachusetts() {
  const fullMode = true; // PRODUCTION: All sites
  console.log('🥕 MASSACHUSETTS MACARONI KID SCRAPER\n' + '='.repeat(60));
  const sitesToScrape = fullMode ? MA_MK_SITES : MA_MK_SITES.slice(0, 5);
  ...
  console.log('\n' + '='.repeat(60));
  console.log('✅ MASSACHUSETTS MACARONI KID SCRAPER COMPLETE!');
  console.log(`📊 Summary:`);
  ...
  await logScraperResult('Macaroni Kid MA', {
    new: imported,
    duplicates: skipped,
    errors: failed
  }, { state: 'MA', source: 'macaroni-kid' });

  return { imported, failed };
}

if (require.main === module) {
  scrapeMacaroniKidMassachusetts()
    .then(() => process.exit(0))
    .catch(error => {
...
module.exports = { scrapeMacaroniKidMassachusetts };
