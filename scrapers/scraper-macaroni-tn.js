...
/**
 * TENNESSEE MACARONI KID SCRAPER
 *
 * Scrapes events from 3 Tennessee Macaroni Kid sites
 *
 * Coverage: Knoxville, SW Nashville, White House-Hendersonville-Mt. Juliet
 *
 * Usage:
 *   node scraper-macaroni-tn.js          # Test mode (5 sites)
 *   node scraper-macaroni-tn.js --full   # Production (all 3 sites)
 */
...
// All 3 Tennessee Macaroni Kid Sites
const TN_MK_SITES = [
  { url: 'https://knoxville.macaronikid.com', name: 'Knoxville', county: 'Knox' },
  { url: 'https://swnashville.macaronikid.com', name: 'SW Nashville', county: 'Davidson' },
  { url: 'https://hendersonvilletn.macaronikid.com', name: 'White House-Hendersonville-Mt. Juliet', county: 'Sumner' }
];

async function geocodeAddress(address, city, zipCode) {
  const fullAddress = `${address}, ${city}, TN ${zipCode}`;
  let result = await tryGeocode(fullAddress);
  if (result) return result;

  const cleaned = address.replace(/,?\s*Suite\s+[A-Z0-9-]+/i, '').replace(/,?\s*#\s*[A-Z0-9-]+/i, '');
  if (cleaned !== address) {
    result = await tryGeocode(`${cleaned}, ${city}, TN ${zipCode}`);
    if (result) return result;
  }

  const streetOnly = cleaned.split(',')[0];
  return await tryGeocode(`${streetOnly}, TN ${zipCode}`);
}
  ...
  try {
    ...
    return await page.evaluate(() => {
      ...
      if (eventsIndex > -1) {
        ...
        if (idx < lines.length) { const cityZipMatch = lines[idx].match(/^([\w\s]+)\s+TN\s+(\d{5})/); if (cityZipMatch) { result.city = cityZipMatch[1].trim(); result.zipCode = cityZipMatch[2]; }}
      }
  ...
  try {
    ...
    for (const url of eventUrls) {
      ...
      if (coords) {
        ...
        if (!coords) {
          const countyCentroid = getCountyCentroid(site.county, 'TN');
          if (countyCentroid) {
      ...
      const eventDoc = {
        ...
        metadata: {
          source: 'Macaroni Kid Tennessee Scraper',
...
async function scrapeMacaroniKidTennessee() {
  const fullMode = true;
  console.log('🥕 TENNESSEE MACARONI KID SCRAPER\n' + '='.repeat(60));
  const sitesToScrape = fullMode ? TN_MK_SITES : TN_MK_SITES.slice(0, 5);
  ...
  console.log('\n' + '='.repeat(60));
  console.log('✅ TENNESSEE MACARONI KID SCRAPER COMPLETE!');
  console.log(`📊 Summary:`);
  ...
  await logScraperResult('Macaroni Kid TN', {
    new: imported,
    duplicates: skipped,
    errors: failed
  }, { state: 'TN', source: 'macaroni-kid' });

  return { imported, failed };
}

if (require.main === module) {
  scrapeMacaroniKidTennessee()
    .then(() => process.exit(0))
    .catch(error => {
...
module.exports = { scrapeMacaroniKidTennessee };
