...
/**
 * MAINE MACARONI KID SCRAPER
 *
 * Scrapes events from 2 Maine Macaroni Kid sites
 *
 * Coverage: Lakes Region~Outer Portland, Midcoast
 *
 * Usage:
 *   node scraper-macaroni-me.js          # Test mode (5 sites)
 *   node scraper-macaroni-me.js --full   # Production (all 2 sites)
 */
...
// All 2 Maine Macaroni Kid Sites
const ME_MK_SITES = [
  { url: 'https://lakesregion.macaronikid.com', name: 'Lakes Region~Outer Portland', county: 'Cumberland' },
  { url: 'https://midcoastme.macaronikid.com', name: 'Midcoast', county: 'Lincoln' }
];

async function geocodeAddress(address, city, zipCode) {
  const fullAddress = `${address}, ${city}, ME ${zipCode}`;
  let result = await tryGeocode(fullAddress);
  if (result) return result;

  const cleaned = address.replace(/,?\s*Suite\s+[A-Z0-9-]+/i, '').replace(/,?\s*#\s*[A-Z0-9-]+/i, '');
  if (cleaned !== address) {
    result = await tryGeocode(`${cleaned}, ${city}, ME ${zipCode}`);
    if (result) return result;
  }

  const streetOnly = cleaned.split(',')[0];
  return await tryGeocode(`${streetOnly}, ME ${zipCode}`);
}
  ...
  try {
    ...
    return await page.evaluate(() => {
      ...
      if (eventsIndex > -1) {
        ...
        if (idx < lines.length) { const cityZipMatch = lines[idx].match(/^([\w\s]+)\s+ME\s+(\d{5})/); if (cityZipMatch) { result.city = cityZipMatch[1].trim(); result.zipCode = cityZipMatch[2]; }}
      }
  ...
  try {
    ...
    for (const url of eventUrls) {
      ...
      if (coords) {
        ...
        if (!coords) {
          const countyCentroid = getCountyCentroid(site.county, 'ME');
          if (countyCentroid) {
...
async function scrapeMacaroniKidMaine() {
  const fullMode = true;
  console.log('🥕 MAINE MACARONI KID SCRAPER\n' + '='.repeat(60));
  const sitesToScrape = fullMode ? ME_MK_SITES : ME_MK_SITES.slice(0, 5);
  ...
  console.log('\n' + '='.repeat(60));
  console.log('✅ MAINE MACARONI KID SCRAPER COMPLETE!');
  console.log(`📊 Summary:`);
  ...
  await logScraperResult('Macaroni Kid ME', {
    ...
    duplicates: skipped,
    errors: failed
  }, { state: 'ME', source: 'macaroni-kid' });

  return { imported, failed };
}

if (require.main === module) {
  scrapeMacaroniKidMaine()
    .then(() => process.exit(0))
    .catch(error => {
...
module.exports = { scrapeMacaroniKidMaine };
