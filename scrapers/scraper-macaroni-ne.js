...
/**
 * NEBRASKA MACARONI KID SCRAPER
 *
 * Scrapes events from 2 Nebraska Macaroni Kid sites
 *
 * Coverage: Lincoln, West Omaha
 *
 * Usage:
 *   node scraper-macaroni-ne.js          # Test mode (5 sites)
 *   node scraper-macaroni-ne.js --full   # Production (all 2 sites)
 */
...
// All 2 Nebraska Macaroni Kid Sites
const NE_MK_SITES = [
  { url: 'https://lincoln.macaronikid.com', name: 'Lincoln', county: 'Lancaster' },
  { url: 'https://westomaha.macaronikid.com', name: 'West Omaha', county: 'Douglas' }
];

async function geocodeAddress(address, city, zipCode) {
  const fullAddress = `${address}, ${city}, NE ${zipCode}`;
  let result = await tryGeocode(fullAddress);
  if (result) return result;

  const cleaned = address.replace(/,?\s*Suite\s+[A-Z0-9-]+/i, '').replace(/,?\s*#\s*[A-Z0-9-]+/i, '');
  if (cleaned !== address) {
    result = await tryGeocode(`${cleaned}, ${city}, NE ${zipCode}`);
    if (result) return result;
  }

  const streetOnly = cleaned.split(',')[0];
  return await tryGeocode(`${streetOnly}, NE ${zipCode}`);
}
  ...
  try {
    ...
    return await page.evaluate(() => {
      ...
      if (eventsIndex > -1) {
        ...
        if (idx < lines.length) { const cityZipMatch = lines[idx].match(/^([\w\s]+)\s+NE\s+(\d{5})/); if (cityZipMatch) { result.city = cityZipMatch[1].trim(); result.zipCode = cityZipMatch[2]; }}
      }
  ...
  try {
    ...
    for (const url of eventUrls) {
      ...
      if (coords) {
        ...
        if (!coords) {
          const countyCentroid = getCountyCentroid(site.county, 'NE');
          if (countyCentroid) {
      ...
      const eventDoc = {
        ...
        metadata: {
          source: 'Macaroni Kid Nebraska Scraper',
...
async function scrapeMacaroniKidNebraska() {
  const fullMode = true;
  console.log('🥕 NEBRASKA MACARONI KID SCRAPER\n' + '='.repeat(60));
  const sitesToScrape = fullMode ? NE_MK_SITES : NE_MK_SITES.slice(0, 5);
  const maxEventsPerSite = fullMode ? 999 : 30; // No limit - filtered by 60 day date range
  console.log(`Mode: ${fullMode ? 'FULL (all 2 sites)' : 'LIMITED (5 sites)'}\nDate range: Next 60 days\n`);
  ...
  console.log('\n' + '='.repeat(60));
  console.log('✅ NEBRASKA MACARONI KID SCRAPER COMPLETE!');
  console.log(`📊 Summary:`);
  ...
  await logScraperResult('Macaroni Kid NE', {
    new: imported,
    duplicates: skipped,
    errors: failed
  }, { state: 'NE', source: 'macaroni-kid' });

  return { imported, failed };
}

if (require.main === module) {
  scrapeMacaroniKidNebraska()
    .then(() => process.exit(0))
    .catch(error => {
...
module.exports = { scrapeMacaroniKidNebraska };
