...
/**
 * ILLINOIS MACARONI KID SCRAPER
 *
 * Scrapes events from 12 Illinois Macaroni Kid sites
 *
 * Coverage: Bloomington-Normal, Chicago Midtown, Chicago Northside, Downers Grove-La Grange-Elmhurst, Gurnee, Naperville, Peoria, Schaumburg-Hoffman Estates, Skokie-Niles-Park Ridge-Far Northwest Chicago, Springfield IL, Wheaton, Yorkville-Geneva-St. Charles
 *
 * Usage:
 *   node scraper-macaroni-il.js          # Test mode (5 sites)
 *   node scraper-macaroni-il.js --full   # Production (all 12 sites)
 */
...
// All 12 Illinois Macaroni Kid Sites
const IL_MK_SITES = [
  { url: 'https://bloomington-normal.macaronikid.com', name: 'Bloomington-Normal', county: 'McLean' },
  { url: 'https://chicagoloop.macaronikid.com', name: 'Chicago Midtown', county: 'Cook' },
  { url: 'https://northsidechicago.macaronikid.com', name: 'Chicago Northside', county: 'Cook' },
  { url: 'https://downersgrove.macaronikid.com', name: 'Downers Grove-La Grange-Elmhurst', county: 'DuPage' },
  { url: 'https://gurnee.macaronikid.com', name: 'Gurnee', county: 'Lake' },
  { url: 'https://naperville.macaronikid.com', name: 'Naperville', county: 'DuPage' },
  { url: 'https://peoria.macaronikid.com', name: 'Peoria', county: 'Peoria' },
  { url: 'https://schaumburg.macaronikid.com', name: 'Schaumburg-Hoffman Estates', county: 'Cook' },
  { url: 'https://skokie.macaronikid.com', name: 'Skokie-Niles-Park Ridge-Far Northwest Chicago', county: 'Cook' },
  { url: 'https://springfieldil.macaronikid.com', name: 'Springfield IL', county: 'Sangamon' },
  { url: 'https://wheaton.macaronikid.com', name: 'Wheaton', county: 'DuPage' },
  { url: 'https://yorkvilleil.macaronikid.com', name: 'Yorkville-Geneva-St. Charles', county: 'Kane' }
];

async function geocodeAddress(address, city, zipCode) {
  const fullAddress = `${address}, ${city}, IL ${zipCode}`;
  let result = await tryGeocode(fullAddress);
  if (result) return result;

  const cleaned = address.replace(/,?\s*Suite\s+[A-Z0-9-]+/i, '').replace(/,?\s*#\s*[A-Z0-9-]+/i, '');
  if (cleaned !== address) {
    result = await tryGeocode(`${cleaned}, ${city}, IL ${zipCode}`);
    if (result) return result;
  }
  ...
  try {
    ...
    return await page.evaluate(() => {
      ...
      if (eventsIndex > -1) {
        ...
        if (idx < lines.length) { const cityZipMatch = lines[idx].match(/^([\w\s]+)\s+IL\s+(\d{5})/); if (cityZipMatch) { result.city = cityZipMatch[1].trim(); result.zipCode = cityZipMatch[2]; }}
      }
  ...
  try {
    ...
    for (const url of eventUrls) {
      ...
      if (coords) {
        ...
        if (!coords) {
          const countyCentroid = getCountyCentroid(site.county, 'IL');
          if (countyCentroid) {
      ...
      const eventDoc = {
        ...
        metadata: {
          source: 'Macaroni Kid Illinois Scraper',
...
async function scrapeMacaroniKidIllinois() {
  const fullMode = true; // PRODUCTION: All sites
  console.log('🥕 ILLINOIS MACARONI KID SCRAPER\n' + '='.repeat(60));
  const sitesToScrape = fullMode ? IL_MK_SITES : IL_MK_SITES.slice(0, 5);
  const maxEventsPerSite = fullMode ? 999 : 30; // No limit - filtered by 60 day date range
  console.log(`Mode: ${fullMode ? 'FULL (all 12 sites)' : 'LIMITED (5 sites)'}\nDate range: Next 60 days\n`);
  ...
  console.log('\n' + '='.repeat(60));
  console.log('✅ ILLINOIS MACARONI KID SCRAPER COMPLETE!');
  console.log(`📊 Summary:`);
  ...
  await logScraperResult('Macaroni Kid IL', {
    new: imported,
    duplicates: skipped,
    errors: failed
  }, { state: 'IL', source: 'macaroni-kid' });

  return { imported, failed };
}

if (require.main === module) {
  scrapeMacaroniKidIllinois()
    .then(() => process.exit(0))
    .catch(error => {
...
module.exports = { scrapeMacaroniKidIllinois };
