...
/**
 * HAWAII MACARONI KID SCRAPER
 *
 * Scrapes events from 1 Hawaii Macaroni Kid sites
 *
 * Coverage: Maui
 *
 * Usage:
 *   node scraper-macaroni-hi.js          # Test mode (5 sites)
 *   node scraper-macaroni-hi.js --full   # Production (all 1 sites)
 */
...
// All 1 Hawaii Macaroni Kid Sites
const HI_MK_SITES = [
  { url: 'https://maui.macaronikid.com', name: 'Maui', county: 'Maui' }
];

async function geocodeAddress(address, city, zipCode) {
  const fullAddress = `${address}, ${city}, HI ${zipCode}`;
  let result = await tryGeocode(fullAddress);
  if (result) return result;

  const cleaned = address.replace(/,?\s*Suite\s+[A-Z0-9-]+/i, '').replace(/,?\s*#\s*[A-Z0-9-]+/i, '');
  if (cleaned !== address) {
    result = await tryGeocode(`${cleaned}, ${city}, HI ${zipCode}`);
    if (result) return result;
  }

  const streetOnly = cleaned.split(',')[0];
  return await tryGeocode(`${streetOnly}, HI ${zipCode}`);
}
  ...
  try {
    ...
    return await page.evaluate(() => {
      ...
      if (eventsIndex > -1) {
        ...
        if (idx < lines.length) { const cityZipMatch = lines[idx].match(/^([\w\s]+)\s+HI\s+(\d{5})/); if (cityZipMatch) { result.city = cityZipMatch[1].trim(); result.zipCode = cityZipMatch[2]; }}
      }
  ...
  try {
    ...
    for (const url of eventUrls) {
      ...
      if (coords) {
        ...
        if (!coords) {
          const countyCentroid = getCountyCentroid(site.county, 'HI');
          if (countyCentroid) {
      ...
      const eventDoc = {
        ...
        metadata: {
          source: 'Macaroni Kid Hawaii Scraper',
...
async function scrapeMacaroniKidHawaii() {
  const fullMode = true;
  console.log('🥕 HAWAII MACARONI KID SCRAPER\n' + '='.repeat(60));
  const sitesToScrape = fullMode ? HI_MK_SITES : HI_MK_SITES.slice(0, 5);
  ...
  console.log('\n' + '='.repeat(60));
  console.log('✅ HAWAII MACARONI KID SCRAPER COMPLETE!');
  console.log(`📊 Summary:`);
  ...
  await logScraperResult('Macaroni Kid HI', {
    new: imported,
    duplicates: skipped,
    errors: failed
  }, { state: 'HI', source: 'macaroni-kid' });

  return { imported, failed };
}

if (require.main === module) {
  scrapeMacaroniKidHawaii()
    .then(() => process.exit(0))
    .catch(error => {
...
module.exports = { scrapeMacaroniKidHawaii };
