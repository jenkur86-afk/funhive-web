...
/**
 * ARKANSAS MACARONI KID SCRAPER
 *
 * Scrapes events from 3 Arkansas Macaroni Kid sites
 *
 * Coverage: Elkins, Fayetteville, Rogers-Bentonville
 *
 * Usage:
 *   node scraper-macaroni-ar.js          # Test mode (all 3 sites)
 *   node scraper-macaroni-ar.js --full   # Production (all 3 sites)
 */
...
// All 3 Arkansas Macaroni Kid Sites
const ARKANSAS_MK_SITES = [
  { url: 'https://elkins.macaronikid.com', name: 'Elkins', county: 'Washington' },
  { url: 'https://fayetteville.macaronikid.com', name: 'Fayetteville', county: 'Washington' },
  { url: 'https://rogers-bentonville.macaronikid.com', name: 'Rogers-Bentoneville', county: 'Benton' }
];

async function geocodeAddress(address, city, zipCode) {
  const fullAddress = `${address}, ${city}, AR ${zipCode}`;
  let result = await tryGeocode(fullAddress);
  if (result) return result;

  const cleaned = address.replace(/,?\s*Suite\s+[A-Z0-9-]+/i, '').replace(/,?\s*#\s*[A-Z0-9-]+/i, '');
  if (cleaned !== address) {
    result = await tryGeocode(`${cleaned}, ${city}, AR ${zipCode}`);
    if (result) return result;
  }

  const streetOnly = cleaned.split(',')[0];
  return await tryGeocode(`${streetOnly}, AR ${zipCode}`);
}
  ...
  try {
    ...
    return await page.evaluate(() => {
      ...
      if (eventsIndex > -1) {
        ...
        if (idx < lines.length) { const cityZipMatch = lines[idx].match(/^([\w\s]+)\s+AR\s+(\d{5})/); if (cityZipMatch) { result.city = cityZipMatch[1].trim(); result.zipCode = cityZipMatch[2]; }}
      }
...
async function scrapeSite(browser, site, maxEvents = 50) {
  ...
  try {
    ...
    // Increased timeout and wait for full page load
    await page.goto(`${site.url}/events/calendar`, { waitUntil: 'networkidle2', timeout: 30000 });
    await new Promise(resolve => setTimeout(resolve, 2000)); // Wait for JS calendar to render
    ...
    for (const url of eventUrls) {
      ...
      const eventDoc = {
        ...
        metadata: {
          source: 'Macaroni Kid Arkansas Scraper',
...
async function scrapeMacaroniKidArkansas() {
  const fullMode = true; // Use full mode since only 3 sites
  console.log('🥕 ARKANSAS MACARONI KID SCRAPER\n' + '='.repeat(60));
  const sitesToScrape = ARKANSAS_MK_SITES; // Always scrape all sites
  const maxEventsPerSite = 50;
  console.log(`Mode: FULL (all 3 sites)\nDate range: Next 60 days\n`);
  ...
  console.log('\n' + '='.repeat(60));
  console.log('✅ ARKANSAS MACARONI KID SCRAPER COMPLETE!');
  console.log(`📊 Summary:`);
  ...
  await logScraperResult('Macaroni Kid AR', {
    new: imported,
    duplicates: skipped,
    errors: failed
  }, { state: 'AR', source: 'macaroni-kid' });
...
module.exports = { scrapeMacaroniKidArkansas };
