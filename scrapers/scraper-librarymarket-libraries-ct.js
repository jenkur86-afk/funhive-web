...
const LIBRARIES = [
  ...
  {
    name: 'West Hartford Public Library',
    eventsUrl: 'https://westhartford.librarymarket.com/',
    city: 'West Hartford',
    state: 'CT',
    zipCode: '06107'
  },
  {
    ...
    eventsUrl: 'https://www.meridenlibrary.org/events/month',
    city: 'Meriden',
    state: 'CT',
    zipCode: '06450'
  },
  {
    ...
    eventsUrl: 'https://fplct.librarymarket.com/',
    city: 'Fairfield',
    state: 'CT',
    zipCode: '06824'
  }
...
const SCRAPER_NAME = 'librarymarket-CT';
...
async function main() {
  console.log(`\n╔════════════════════════════════════════════════════════╗`);
  console.log(`║  Generic Scraper - CT (${LIBRARIES.length} libraries)  ║`);
  console.log(`╚════════════════════════════════════════════════════════╝\n`);
  ...
  await logScraperResult('Librarymarket Libraries CT', {
 ...
 */
async function scrapeLibraryMarketCTCloudFunction() {
  console.log('☁️ Running librarymarket-CT as Cloud Function');
  const events = await scrapeGenericEvents();
  if (events.length > 0) {
    await saveToDatabase(events);
  }
...
module.exports = { scrapeGenericEvents, saveToDatabase, scrapeLibraryMarketCTCloudFunction };
