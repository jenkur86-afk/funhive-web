...
/**
 * COLORADO MACARONI KID SCRAPER
 *
 * Scrapes events from 15 Colorado Macaroni Kid sites
 *
 * Coverage: Arvada-Wheat Ridge-Golden, Aurora, Boulder, Broomfield-Erie-Lafayette-Louisville, Central Park-Commerce City-Northfield, Colorado Springs, Downtown Denver, Englewood-Greenwood Village-Centennial, Evergreen-Bailey-Conifer, Fort Collins, Highlands Ranch-Parker-Castle Rock-Lone Tree, Lakewood-Littleton, Longmont-Frederick-Niwot-Lyons, Loveland, Thornton-Northglenn-E. Westminster
 *
 * Usage:
 *   node scraper-macaroni-co.js          # Test mode (5 sites)
 *   node scraper-macaroni-co.js --full   # Production (all 15 sites)
 */
...
// All 15 Colorado Macaroni Kid Sites
const CO_MK_SITES = [
  { url: 'https://arvada.macaronikid.com', name: 'Arvada-Wheat Ridge-Golden', county: 'Jefferson' },
  { url: 'https://aurora.macaronikid.com', name: 'Aurora', county: 'Arapahoe' },
  { url: 'https://boulderco.macaronikid.com', name: 'Boulder', county: 'Boulder' },
  { url: 'https://broomfield.macaronikid.com', name: 'Broomfield-Erie-Lafayette-Louisville', county: 'Broomfield' },
  { url: 'https://commercecity.macaronikid.com', name: 'Central Park-Commerce City-Northfield', county: 'Adams' },
  { url: 'https://coloradosprings.macaronikid.com', name: 'Colorado Springs', county: 'El Paso' },
  { url: 'https://downtowndenver.macaronikid.com', name: 'Downtown Denver', county: 'Denver' },
  { url: 'https://englewood.macaronikid.com', name: 'Englewood-Greenwood Village-Centennial', county: 'Arapahoe' },
  { url: 'https://evergreen.macaronikid.com', name: 'Evergreen-Bailey-Conifer', county: 'Jefferson' },
  { url: 'https://fortcollins.macaronikid.com', name: 'Fort Collins', county: 'Larimer' },
  { url: 'https://highlandsranch.macaronikid.com', name: 'Highlands Ranch-Parker-Castle Rock-Lone Tree', county: 'Douglas' },
  { url: 'https://lakewoodco.macaronikid.com', name: 'Lakewood-Littleton', county: 'Jefferson' },
  { url: 'https://longmont.macaronikid.com', name: 'Longmont-Frederick-Niwot-Lyons', county: 'Boulder' },
  { url: 'https://loveland.macaronikid.com', name: 'Loveland', county: 'Larimer' },
  { url: 'https://thornton.macaronikid.com', name: 'Thornton-Northglenn-E. Westminster', county: 'Adams' }
];

async function geocodeAddress(address, city, zipCode) {
  const fullAddress = `${address}, ${city}, CO ${zipCode}`;
  let result = await tryGeocode(fullAddress);
  if (result) return result;

  const cleaned = address.replace(/,?\s*Suite\s+[A-Z0-9-]+/i, '').replace(/,?\s*#\s*[A-Z0-9-]+/i, '');
  if (cleaned !== address) {
    result = await tryGeocode(`${cleaned}, ${city}, CO ${zipCode}`);
    if (result) return result;
  }

  const streetOnly = cleaned.split(',')[0];
  return await tryGeocode(`${streetOnly}, CO ${zipCode}`);
}
  ...
  try {
    ...
    return await page.evaluate(() => {
      ...
      if (eventsIndex > -1) {
        ...
        if (idx < lines.length) { const cityZipMatch = lines[idx].match(/^([\w\s]+)\s+CO\s+(\d{5})/); if (cityZipMatch) { result.city = cityZipMatch[1].trim(); result.zipCode = cityZipMatch[2]; }}
      }
  ...
  try {
    ...
    for (const url of eventUrls) {
      ...
      if (coords) {
        ...
        if (!coords) {
          const countyCentroid = getCountyCentroid(site.county, 'CO');
          if (countyCentroid) {
      ...
      const eventDoc = {
        ...
        metadata: {
          source: 'Macaroni Kid Colorado Scraper',
...
async function scrapeMacaroniKidColorado() {
  const fullMode = true; // PRODUCTION: All sites
  console.log('🥕 COLORADO MACARONI KID SCRAPER\n' + '='.repeat(60));
  const sitesToScrape = fullMode ? CO_MK_SITES : CO_MK_SITES.slice(0, 5);
  const maxEventsPerSite = fullMode ? 999 : 30; // No limit - filtered by 60 day date range
  console.log(`Mode: ${fullMode ? 'FULL (all 15 sites)' : 'LIMITED (5 sites)'}\nDate range: Next 60 days\n`);
  ...
  console.log('\n' + '='.repeat(60));
  console.log('✅ COLORADO MACARONI KID SCRAPER COMPLETE!');
  console.log(`📊 Summary:`);
  ...
  await logScraperResult('Macaroni Kid CO', {
    new: imported,
    duplicates: skipped,
    errors: failed
  }, { state: 'CO', source: 'macaroni-kid' });

  return { imported, failed };
}

if (require.main === module) {
  scrapeMacaroniKidColorado()
    .then(() => process.exit(0))
    .catch(error => {
...
module.exports = { scrapeMacaroniKidColorado };
