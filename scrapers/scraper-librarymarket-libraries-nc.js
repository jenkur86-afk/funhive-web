 ...
 * Libraries:
 *   - Buncombe County Libraries (Asheville, NC)
 *   - Hickory Public Library (Hickory, NC)
 */

const LIBRARIES = [
  {
    name: 'Buncombe County Libraries',
    eventsUrl: 'https://buncombe.librarycalendar.com/events/month',
    city: 'Asheville',
    state: 'NC',
    zipCode: '28801'
  },
  {
    ...
    eventsUrl: 'https://hickory.librarycalendar.com/',
    city: 'Hickory',
    state: 'NC',
    zipCode: '28601'
  }
...
const SCRAPER_NAME = 'librarymarket-NC';
...
async function main() {
  console.log(`\n╔════════════════════════════════════════════════════════╗`);
  console.log(`║  Generic Scraper - NC (${LIBRARIES.length} libraries)  ║`);
  console.log(`╚════════════════════════════════════════════════════════╝\n`);
  ...
  await logScraperResult('Librarymarket Libraries NC', {
 ...
 */
async function scrapeLibraryMarketNCCloudFunction() {
  console.log('☁️ Running librarymarket-NC as Cloud Function');
  const events = await scrapeGenericEvents();
  if (events.length > 0) {
    await saveToDatabase(events);
  }
...
module.exports = { scrapeGenericEvents, saveToDatabase, scrapeLibraryMarketNCCloudFunction };
