...
async function scrapeSite(browser, site, logger, maxEvents = 50) {
  ...
  try {
    ...
    let imported = 0, updated = 0, skippedPast = 0, skippedFuture = 0, failedExtract = 0, noLocation = 0, skippedDuplicate = 0;
    ...
    console.log(`  ✅ ${imported} new | 🔄 ${updated} updated | 🔁 ${skippedDuplicate} dup | ⏭️ ${skippedPast} past | ⏩ ${skippedFuture} future | ❓ ${failedExtract} no-date | ⚠️ ${noLocation} no coords`);
    await page.close();
  } catch (error) {
...
async function scrapeMacaroniKidMaryland() {
  ...
  // Log to database with per-site breakdown
  await logger.finish();
