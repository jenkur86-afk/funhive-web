...
/**
 * MACARONI KID RUNNER - GROUP 1
 * Runs MacaroniKid scrapers for Group 1 states only
 *
 * Schedule: Days 1, 4, 7, 10, 13, 16, 19, 22, 25, 28, 31
 *
 * Usage:
 *   node macaroni-runner-group1.js                 # Run all Group 1 states
 *   node macaroni-runner-group1.js --dry-run       # Preview without running
 *   node macaroni-runner-group1.js --state CA      # Run specific state
 *
...
const CONFIG = {
  ...
  // Log file
  LOG_FILE: path.join(__dirname, '..', 'logs', `macaroni-group1-${new Date().toISOString().split('T')[0]}.log`)
};
...
async function logToSupabase(scraperName, status, stats, error, executionTime) {
  try {
    await saveScraperLog({
      scraperName: `MacaroniKid-Group1-${scraperName}`,
...
async function main() {
  ...
  if (options.help) {
    console.log(`
MacaroniKid Runner - Group 1

Usage:
  node macaroni-runner-group1.js                 # Run all Group 1 states
  node macaroni-runner-group1.js --state CA      # Run specific state
  node macaroni-runner-group1.js --dry-run       # Preview without running
  node macaroni-runner-group1.js --help          # Show this help

Schedule: Days 1, 4, 7, 10, 13, 16, 19, 22, 25, 28, 31

Group 1 States: ${Object.values(MACARONI_SCRAPERS)
  ...
  // `node macaroni-runner-group1.js --state CA`) get a labeled table too.
