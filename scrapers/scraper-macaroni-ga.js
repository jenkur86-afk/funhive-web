...
/**
 * GEORGIA MACARONI KID SCRAPER
 *
 * Scrapes events from 17 Georgia Macaroni Kid sites
 *
 * Coverage: Alpharetta-Roswell-Milton, Athens, Buckhead-Midtown-Brookhaven, College Park-East Point-Jonesboro, Decatur-Toco Hills-Avondale Estates, Duluth-Norcross-Suwanee-Johns Creek-Ptree Corners, East Atlanta-Downtown Atlanta-Little 5 Points, Elberton, Franklin-Hart, Gainesville-Flowery Branch, Lawrenceville, Peachtree City-Fayetteville-Newnan, Sandy Springs-Dunwoody, Smyrna Vinings, Snellville-Grayson-Stone Mtn, Stonecrest-Conyers-Covington, Winder-Buford
 *
 * Usage:
 *   node scraper-macaroni-ga.js          # Test mode (5 sites)
 *   node scraper-macaroni-ga.js --full   # Production (all 17 sites)
 */
...
// All 17 Georgia Macaroni Kid Sites
const GA_MK_SITES = [
  { url: 'https://alpharetta.macaronikid.com', name: 'Alpharetta-Roswell-Milton', county: 'Fulton' },
  { url: 'https://athens.macaronikid.com', name: 'Athens', county: 'Clarke' },
  { url: 'https://buckheadbrookhaven.macaronikid.com', name: 'Buckhead-Midtown-Brookhaven', county: 'Fulton' },
  { url: 'https://collegepark.macaronikid.com', name: 'College Park-East Point-Jonesboro', county: 'Clayton' },
  { url: 'https://decatur.macaronikid.com', name: 'Decatur-Toco Hills-Avondale Estates', county: 'DeKalb' },
  { url: 'https://duluth.macaronikid.com', name: 'Duluth-Norcross-Suwanee-Johns Creek-Ptree Corners', county: 'Gwinnett' },
  { url: 'https://eastatlanta.macaronikid.com', name: 'East Atlanta-Downtown Atlanta-Little 5 Points', county: 'Fulton' },
  { url: 'https://elberton.macaronikid.com', name: 'Elberton', county: 'Elbert' },
  { url: 'https://franklinga.macaronikid.com', name: 'Franklin-Hart', county: 'Hart' },
  { url: 'https://gainesvillega.macaronikid.com', name: 'Gainesville-Flowery Branch', county: 'Hall' },
  { url: 'https://lawrenceville.macaronikid.com', name: 'Lawrenceville', county: 'Gwinnett' },
  { url: 'https://peachtreecity.macaronikid.com', name: 'Peachtree City-Fayetteville-Newnan', county: 'Fayette' },
  { url: 'https://sandysprings.macaronikid.com', name: 'Sandy Springs-Dunwoody', county: 'Fulton' },
  { url: 'https://smyrnavinings.macaronikid.com', name: 'Smyrna Vinings', county: 'Cobb' },
  { url: 'https://snellville.macaronikid.com', name: 'Snellville-Grayson-Stone Mtn', county: 'Gwinnett' },
  { url: 'https://conyers.macaronikid.com', name: 'Stonecrest-Conyers-Covington', county: 'Rockdale' },
  { url: 'https://winder.macaronikid.com', name: 'Winder-Buford', county: 'Barrow' }
];
...
async function geocodeAddress(address, city, zipCode) {
  ...
  if (cleaned !== address) {
    result = await tryGeocode(`${cleaned}, ${city}, GA ${zipCode}`);
    if (result) return result;
  }

  const streetOnly = cleaned.split(',')[0];
  return await tryGeocode(`${streetOnly}, GA ${zipCode}`);
}
  ...
  try {
    ...
    return await page.evaluate(() => {
      ...
      if (eventsIndex > -1) {
        ...
        if (idx < lines.length) { const cityZipMatch = lines[idx].match(/^([\w\s]+)\s+GA\s+(\d{5})/); if (cityZipMatch) { result.city = cityZipMatch[1].trim(); result.zipCode = cityZipMatch[2]; }}
      }
  ...
  try {
    ...
    for (const url of eventUrls) {
      ...
      if (coords) {
        ...
        if (!coords) {
          const countyCentroid = getCountyCentroid(site.county, 'GA');
          if (countyCentroid) {
      ...
      const eventDoc = {
        ...
        metadata: {
          source: 'Macaroni Kid Georgia Scraper',
...
async function scrapeMacaroniKidGeorgia() {
  const fullMode = true; // PRODUCTION: All sites
  console.log('🥕 GEORGIA MACARONI KID SCRAPER\n' + '='.repeat(60));
  const sitesToScrape = fullMode ? GA_MK_SITES : GA_MK_SITES.slice(0, 5);
  ...
  console.log('\n' + '='.repeat(60));
  console.log('✅ GEORGIA MACARONI KID SCRAPER COMPLETE!');
  console.log(`📊 Summary:`);
  ...
  await logScraperResult('Macaroni Kid GA', {
    new: imported,
    duplicates: skipped,
    errors: failed
  }, { state: 'GA', source: 'macaroni-kid' });

  return { imported, failed };
}

if (require.main === module) {
  scrapeMacaroniKidGeorgia()
    .then(() => process.exit(0))
    .catch(error => {
...
module.exports = { scrapeMacaroniKidGeorgia };
