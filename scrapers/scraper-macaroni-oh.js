...
/**
 * OHIO MACARONI KID SCRAPER
 *
 * Scrapes events from 7 Ohio Macaroni Kid sites
 *
 * Coverage: Cleveland East, Cleveland West, Medina County, NE Columbus, NW Columbus, North Akron, Sandusky
 *
 * Usage:
 *   node scraper-macaroni-oh.js          # Test mode (5 sites)
 *   node scraper-macaroni-oh.js --full   # Production (all 7 sites)
 */
...
// All 7 Ohio Macaroni Kid Sites
const OH_MK_SITES = [
  { url: 'https://clevelandeast.macaronikid.com', name: 'Cleveland East', county: 'Cuyahoga' },
  { url: 'https://clevelandwest.macaronikid.com', name: 'Cleveland West', county: 'Cuyahoga' },
  { url: 'https://medina.macaronikid.com', name: 'Medina County', county: 'Medina' },
  { url: 'https://necolumbus.macaronikid.com', name: 'NE Columbus', county: 'Franklin' },
  { url: 'https://nwcolumbus.macaronikid.com', name: 'NW Columbus', county: 'Franklin' },
  { url: 'https://northakron.macaronikid.com', name: 'North Akron', county: 'Summit' },
  { url: 'https://sandusky.macaronikid.com', name: 'Sandusky', county: 'Erie' }
];

async function geocodeAddress(address, city, zipCode) {
  return _sharedGeocodeAddress(address, city, 'OH', zipCode);
}

async function geocodeVenue(venue, city, zipCode) {
  return _sharedGeocodeVenue(venue, city, 'OH', zipCode);
}
    ...
    for (const url of eventUrls) {
      ...
      if (coords) {
        ...
        if (!coords && details.city) {
          coords = await getCityCenterCoords(details.city, 'OH', details.zipCode);
          if (coords) {
            locationObj = {
              address: details.address || '',
              city: details.city,
        ...
        if (!coords) {
          const countyCentroid = getCountyCentroid(site.county, 'OH');
          if (countyCentroid) {
      ...
      // Use the already-parsed eventDate from line 228 (new Date(details.eventDate))
      // Append T00:00:00 to avoid UTC midnight timezone shift
      ...
      const eventDoc = {
        ...
        metadata: {
          source: 'Macaroni Kid Ohio Scraper',
          sourceUrl: site.url,
          scraperName: `MacaroniKid-OH-${site.url.replace(/^https?:\/\//, '').replace(/^www\./, '').split('.')[0]}`,
...
async function scrapeMacaroniKidOhio() {
  const fullMode = true; // PRODUCTION: All sites
  console.log('🥕 OHIO MACARONI KID SCRAPER\n' + '='.repeat(60));
  const sitesToScrape = fullMode ? OH_MK_SITES : OH_MK_SITES.slice(0, 5);
  const maxEventsPerSite = fullMode ? 999 : 30; // No limit - filtered by 60 day date range
  console.log(`Mode: ${fullMode ? 'FULL (all 7 sites)' : 'LIMITED (5 sites)'}\nDate range: Next 60 days\n`);
  ...
  console.log('\n' + '='.repeat(60));
  console.log('✅ OHIO MACARONI KID SCRAPER COMPLETE!');
  console.log(`📊 Summary:`);
  ...
  await logScraperResult('Macaroni Kid OH', {
    new: imported,
    duplicates: skipped,
    errors: failed
  }, { state: 'OH', source: 'macaroni-kid' });

  return { imported, failed };
}

if (require.main === module) {
  scrapeMacaroniKidOhio()
    .then(() => process.exit(0))
    .catch(error => {
...
module.exports = { scrapeMacaroniKidOhio };
