...
const LIBRARIES = [
  {
    ...
    eventsUrl: 'https://arcpls.librarycalendar.com/events/upcoming',
    city: 'Augusta',
    state: 'GA',
    zipCode: '30901'
  }
...
const SCRAPER_NAME = 'librarymarket-GA';
...
async function main() {
  console.log(`\n╔════════════════════════════════════════════════════════╗`);
  console.log(`║  Generic Scraper - GA (${LIBRARIES.length} libraries)  ║`);
  console.log(`╚════════════════════════════════════════════════════════╝\n`);
  ...
  await logScraperResult('Librarymarket Libraries GA', {
 ...
 */
async function scrapeLibraryMarketGACloudFunction() {
  console.log('☁️ Running librarymarket-GA as Cloud Function');
  const events = await scrapeGenericEvents();
  if (events.length > 0) {
    await saveToDatabase(events);
  }
...
module.exports = { scrapeGenericEvents, saveToDatabase, scrapeLibraryMarketGACloudFunction };
