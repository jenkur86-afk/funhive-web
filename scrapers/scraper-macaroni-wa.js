...
/**
 * WASHINGTON MACARONI KID SCRAPER
 *
 * Scrapes events from 11 Washington Macaroni Kid sites
 *
 * Coverage: Everett-Marysville-Snohomish-Lake Stevens-Mukilteo, Gig Harbor, Kitsap, Maple Valley, Monroe-Duvall-Carnation-Clearview, North Sound, Olympic Peninsula, Redmond-Kirkland-Bothell-Woodinville, Snoqualmie Valley-Issaquah-Sammamish, Spokane, Whidbey Island
 *
 * Usage:
 *   node scraper-macaroni-wa.js          # Test mode (5 sites)
 *   node scraper-macaroni-wa.js --full   # Production (all 11 sites)
 */
...
// All 11 Washington Macaroni Kid Sites
const WA_MK_SITES = [
  { url: 'https://marysville.macaronikid.com', name: 'Everett-Marysville-Snohomish-Lake Stevens-Mukilteo', county: 'Snohomish' },
  { url: 'https://gigharbor.macaronikid.com', name: 'Gig Harbor', county: 'Pierce' },
  { url: 'https://kitsap.macaronikid.com', name: 'Kitsap', county: 'Kitsap' },
  { url: 'https://maplevalley.macaronikid.com', name: 'Maple Valley', county: 'King' },
  { url: 'https://cherryvalley.macaronikid.com', name: 'Monroe-Duvall-Carnation-Clearview', county: 'Snohomish' },
  { url: 'https://northsound.macaronikid.com', name: 'North Sound', county: 'Skagit' },
  { url: 'https://olympicpeninsula.macaronikid.com', name: 'Olympic Peninsula', county: 'Jefferson' },
  { url: 'https://the-eastside.macaronikid.com', name: 'Redmond-Kirkland-Bothell-Woodinville', county: 'King' },
  { url: 'https://snoqualmievalley.macaronikid.com', name: 'Snoqualmie Valley-Issaquah-Sammamish', county: 'King' },
  { url: 'https://spokane.macaronikid.com', name: 'Spokane', county: 'Spokane' },
  { url: 'https://whidbeyisland.macaronikid.com', name: 'Whidbey Island', county: 'Island' }
];

async function geocodeAddress(address, city, zipCode) {
  const fullAddress = `${address}, ${city}, WA ${zipCode}`;
  let result = await tryGeocode(fullAddress);
  if (result) return result;

  const cleaned = address.replace(/,?\s*Suite\s+[A-Z0-9-]+/i, '').replace(/,?\s*#\s*[A-Z0-9-]+/i, '');
  if (cleaned !== address) {
    result = await tryGeocode(`${cleaned}, ${city}, WA ${zipCode}`);
    if (result) return result;
  }

  const streetOnly = cleaned.split(',')[0];
  return await tryGeocode(`${streetOnly}, WA ${zipCode}`);
}
  ...
  try {
    ...
    return await page.evaluate(() => {
      ...
      if (eventsIndex > -1) {
        ...
        if (idx < lines.length) { const cityZipMatch = lines[idx].match(/^([\w\s]+)\s+WA\s+(\d{5})/); if (cityZipMatch) { result.city = cityZipMatch[1].trim(); result.zipCode = cityZipMatch[2]; }}
      }
  ...
  try {
    ...
    for (const url of eventUrls) {
      ...
      if (coords) {
        ...
        if (!coords) {
          const countyCentroid = getCountyCentroid(site.county, 'WA');
          if (countyCentroid) {
      ...
      const eventDoc = {
        ...
        metadata: {
          source: 'Macaroni Kid Washington Scraper',
...
async function scrapeMacaroniKidWashington() {
  const fullMode = true; // PRODUCTION: All sites
  console.log('🥕 WASHINGTON MACARONI KID SCRAPER\n' + '='.repeat(60));
  const sitesToScrape = fullMode ? WA_MK_SITES : WA_MK_SITES.slice(0, 5);
  const maxEventsPerSite = fullMode ? 999 : 30; // No limit - filtered by 60 day date range
  console.log(`Mode: ${fullMode ? 'FULL (all 11 sites)' : 'LIMITED (5 sites)'}\nDate range: Next 60 days\n`);
  ...
  console.log('\n' + '='.repeat(60));
  console.log('✅ WASHINGTON MACARONI KID SCRAPER COMPLETE!');
  console.log(`📊 Summary:`);
  ...
  await logScraperResult('Macaroni Kid WA', {
    new: imported,
    duplicates: skipped,
    errors: failed
  }, { state: 'WA', source: 'macaroni-kid' });

  return { imported, failed };
}

if (require.main === module) {
  scrapeMacaroniKidWashington()
    .then(() => process.exit(0))
    .catch(error => {
...
module.exports = { scrapeMacaroniKidWashington };
