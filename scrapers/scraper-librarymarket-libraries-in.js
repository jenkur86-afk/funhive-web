...
const LIBRARIES = [
  {
    name: 'Hammond Public Library',
    eventsUrl: 'https://hammondpl.librarycalendar.com/events/month',
    city: 'Hammond',
    state: 'IN',
    zipCode: '46320'
  },
  {
    name: 'Vigo County Public Library',
    eventsUrl: 'https://vigocounty.librarycalendar.com/events/month',
    city: 'Terre Haute',
    state: 'IN',
    zipCode: '47807'
  },
  {
    ...
    eventsUrl: 'https://tippecanoe.librarycalendar.com/',
    city: 'Lafayette',
    state: 'IN',
    zipCode: '47901'
  }
...
const SCRAPER_NAME = 'librarymarket-IN';
...
async function main() {
  console.log(`\n╔════════════════════════════════════════════════════════╗`);
  console.log(`║  Generic Scraper - IN (${LIBRARIES.length} libraries)  ║`);
  console.log(`╚════════════════════════════════════════════════════════╝\n`);
  ...
  await logScraperResult('Librarymarket Libraries IN', {
 ...
 */
async function scrapeLibraryMarketINCloudFunction() {
  console.log('☁️ Running librarymarket-IN as Cloud Function');
  const events = await scrapeGenericEvents();
  if (events.length > 0) {
    await saveToDatabase(events);
  }
...
module.exports = { scrapeGenericEvents, saveToDatabase, scrapeLibraryMarketINCloudFunction };
