...
/**
 * KENTUCKY MACARONI KID SCRAPER
 *
 * Scrapes events from 4 Kentucky Macaroni Kid sites
 *
 * Coverage: Lexington, Louisville Central, Louisville East-Oldham County, Louisville SW
 *
 * Usage:
 *   node scraper-macaroni-ky.js          # Test mode (5 sites)
 *   node scraper-macaroni-ky.js --full   # Production (all 4 sites)
 */
...
// All 4 Kentucky Macaroni Kid Sites
const KY_MK_SITES = [
  { url: 'https://lexingtonky.macaronikid.com', name: 'Lexington', county: 'Fayette' },
  { url: 'https://centrallouisville.macaronikid.com', name: 'Louisville Central', county: 'Jefferson' },
  { url: 'https://louisvilleeast.macaronikid.com', name: 'Louisville East-Oldham County', county: 'Oldham' },
  { url: 'https://louisvillesw.macaronikid.com', name: 'Louisville SW', county: 'Jefferson' }
];

async function geocodeAddress(address, city, zipCode) {
  const fullAddress = `${address}, ${city}, KY ${zipCode}`;
  let result = await tryGeocode(fullAddress);
  if (result) return result;

  const cleaned = address.replace(/,?\s*Suite\s+[A-Z0-9-]+/i, '').replace(/,?\s*#\s*[A-Z0-9-]+/i, '');
  if (cleaned !== address) {
    result = await tryGeocode(`${cleaned}, ${city}, KY ${zipCode}`);
    if (result) return result;
  }

  const streetOnly = cleaned.split(',')[0];
  return await tryGeocode(`${streetOnly}, KY ${zipCode}`);
}
  ...
  try {
    ...
    return await page.evaluate(() => {
      ...
      if (eventsIndex > -1) {
        ...
        if (idx < lines.length) { const cityZipMatch = lines[idx].match(/^([\w\s]+)\s+KY\s+(\d{5})/); if (cityZipMatch) { result.city = cityZipMatch[1].trim(); result.zipCode = cityZipMatch[2]; }}
      }
  ...
  try {
    ...
    for (const url of eventUrls) {
      ...
      if (coords) {
        ...
        if (!coords) {
          const countyCentroid = getCountyCentroid(site.county, 'KY');
          if (countyCentroid) {
      ...
      const eventDoc = {
        ...
        metadata: {
          source: 'Macaroni Kid Kentucky Scraper',
...
async function scrapeMacaroniKidKentucky() {
  const fullMode = true;
  console.log('🥕 KENTUCKY MACARONI KID SCRAPER\n' + '='.repeat(60));
  const sitesToScrape = fullMode ? KY_MK_SITES : KY_MK_SITES.slice(0, 5);
  const maxEventsPerSite = fullMode ? 999 : 30; // No limit - filtered by 60 day date range
  console.log(`Mode: ${fullMode ? 'FULL (all 4 sites)' : 'LIMITED (5 sites)'}\nDate range: Next 60 days\n`);
  ...
  console.log('\n' + '='.repeat(60));
  console.log('✅ KENTUCKY MACARONI KID SCRAPER COMPLETE!');
  console.log(`📊 Summary:`);
  ...
  await logScraperResult('Macaroni Kid KY', {
    new: imported,
    duplicates: skipped,
    errors: failed
  }, { state: 'KY', source: 'macaroni-kid' });

  return { imported, failed };
}

if (require.main === module) {
  scrapeMacaroniKidKentucky()
    .then(() => process.exit(0))
    .catch(error => {
...
module.exports = { scrapeMacaroniKidKentucky };
