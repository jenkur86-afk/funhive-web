...
const LIBRARIES = [
  {
    ...
    eventsUrl: 'https://rochesterpubliclibrary.librarymarket.com/',
    city: 'Rochester',
    state: 'MN',
    zipCode: '55901'
  }
...
const SCRAPER_NAME = 'librarymarket-MN';
...
async function main() {
  console.log(`\n╔════════════════════════════════════════════════════════╗`);
  console.log(`║  Generic Scraper - MN (${LIBRARIES.length} libraries)  ║`);
  console.log(`╚════════════════════════════════════════════════════════╝\n`);
  ...
  await logScraperResult('Librarymarket Libraries MN', {
 ...
 */
async function scrapeLibraryMarketMNCloudFunction() {
  console.log('☁️ Running librarymarket-MN as Cloud Function');
  const events = await scrapeGenericEvents();
  if (events.length > 0) {
    await saveToDatabase(events);
  }
...
module.exports = { scrapeGenericEvents, saveToDatabase, scrapeLibraryMarketMNCloudFunction };
