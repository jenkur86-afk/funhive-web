...
/**
 * NEW JERSEY MACARONI KID SCRAPER
 *
 * Scrapes events from 15 New Jersey Macaroni Kid sites
 *
 * Coverage: Cherry Hill, Clark-Cranford-Scotch Plains, Clifton-Montclair, East Morris, Edison-Highland Park-Metuchen, Lincroft-Holmdel-Tinton Falls, Little Egg-LBI-Stafford-Barnegat-Waretown, New Brunswick-Somerset, Northwest Bergen, Point Pleasant NJ, Sussex, Toms River, Warren-Bernards, Western Monmouth, West Morris
 *
 * Usage:
 *   node scraper-macaroni-nj.js          # Test mode (5 sites)
 *   node scraper-macaroni-nj.js --full   # Production (all 15 sites)
 */
...
// All 15 New Jersey Macaroni Kid Sites
const NJ_MK_SITES = [
  { url: 'https://cherryhill.macaronikid.com', name: 'Cherry Hill', county: 'Camden' },
  { url: 'https://clark.macaronikid.com', name: 'Clark-Cranford-Scotch Plains', county: 'Union' },
  { url: 'https://clifton.macaronikid.com', name: 'Clifton-Montclair', county: 'Essex' },
  { url: 'https://eastmorris.macaronikid.com', name: 'East Morris', county: 'Morris' },
  { url: 'https://edison.macaronikid.com', name: 'Edison-Highland Park-Metuchen', county: 'Middlesex' },
  { url: 'https://tintonfalls.macaronikid.com', name: 'Lincroft-Holmdel-Tinton Falls', county: 'Monmouth' },
  { url: 'https://littleeggharbor.macaronikid.com', name: 'Little Egg-LBI-Stafford-Barnegat-Waretown', county: 'Ocean' },
  { url: 'https://newbrunswick.macaronikid.com', name: 'New Brunswick-Somerset', county: 'Middlesex' },
  { url: 'https://ramsey.macaronikid.com', name: 'Northwest Bergen', county: 'Bergen' },
  { url: 'https://pointpleasantnj.macaronikid.com', name: 'Point Pleasant NJ', county: 'Ocean' },
  { url: 'https://sussexnj.macaronikid.com', name: 'Sussex', county: 'Sussex' },
  { url: 'https://tomsriver.macaronikid.com', name: 'Toms River', county: 'Ocean' },
  { url: 'https://warren.macaronikid.com', name: 'Warren-Bernards', county: 'Somerset' },
  { url: 'https://marlboro-manalapan.macaronikid.com', name: 'Western Monmouth', county: 'Monmouth' },
  { url: 'https://westmorris.macaronikid.com', name: 'West Morris', county: 'Morris' }
];
...
async function geocodeAddress(address, city, zipCode) {
  ...
  if (cleaned !== address) {
    result = await tryGeocode(`${cleaned}, ${city}, NJ ${zipCode}`);
    if (result) return result;
  }

  const streetOnly = cleaned.split(',')[0];
  return await tryGeocode(`${streetOnly}, NJ ${zipCode}`);
}
  ...
  try {
    ...
    return await page.evaluate(() => {
      ...
      if (eventsIndex > -1) {
        ...
        if (idx < lines.length) { const cityZipMatch = lines[idx].match(/^([\w\s]+)\s+NJ\s+(\d{5})/); if (cityZipMatch) { result.city = cityZipMatch[1].trim(); result.zipCode = cityZipMatch[2]; }}
      }
  ...
  try {
    ...
    for (const url of eventUrls) {
      ...
      if (coords) {
        ...
        if (!coords) {
          const countyCentroid = getCountyCentroid(site.county, 'NJ');
          if (countyCentroid) {
      ...
      const eventDoc = {
        ...
        metadata: {
          source: 'Macaroni Kid New Jersey Scraper',
...
async function scrapeMacaroniKidNewJersey() {
  const fullMode = true; // PRODUCTION: All sites
  console.log('🥕 NEW JERSEY MACARONI KID SCRAPER\n' + '='.repeat(60));
  const sitesToScrape = fullMode ? NJ_MK_SITES : NJ_MK_SITES.slice(0, 5);
  ...
  console.log('\n' + '='.repeat(60));
  console.log('✅ NEW JERSEY MACARONI KID SCRAPER COMPLETE!');
  console.log(`📊 Summary:`);
  ...
  await logScraperResult('Macaroni Kid NJ', {
    new: imported,
    duplicates: skipped,
    errors: failed
  }, { state: 'NJ', source: 'macaroni-kid' });

  return { imported, failed };
}

if (require.main === module) {
  scrapeMacaroniKidNewJersey()
    .then(() => process.exit(0))
    .catch(error => {
...
module.exports = { scrapeMacaroniKidNewJersey };
