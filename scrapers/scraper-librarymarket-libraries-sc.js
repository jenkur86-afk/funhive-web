...
const LIBRARIES = [
  {
    name: 'Sumter County Library',
    eventsUrl: 'https://sumtercounty.librarycalendar.com/',
    city: 'Sumter',
    state: 'SC',
    zipCode: '29150'
  },
  {
    ...
    eventsUrl: 'https://beaufort.librarycalendar.com/events/upcoming',
    city: 'Beaufort',
    state: 'SC',
    zipCode: '29902'
  }
...
async function main() {
  console.log(`\n╔════════════════════════════════════════════════════════╗`);
  console.log(`║  Generic Scraper - SC (${LIBRARIES.length} libraries)  ║`);
  console.log(`╚════════════════════════════════════════════════════════╝\n`);
  ...
  await logScraperResult('Librarymarket Libraries SC', {
 ...
 */
async function scrapeLibraryMarketSCCloudFunction() {
  console.log('☁️ Running librarymarket-SC as Cloud Function');
  const events = await scrapeGenericEvents();
  if (events.length > 0) {
    await saveToDatabase(events);
  }
...
module.exports = { scrapeGenericEvents, saveToDatabase, scrapeLibraryMarketSCCloudFunction };
