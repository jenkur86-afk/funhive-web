...
-- Composite index for geospatial queries on events with a date
-- (removed WHERE date >= NOW() — partial indexes require IMMUTABLE expressions)
CREATE INDEX idx_events_date_location ON events USING GIST (location) WHERE date IS NOT NULL;
