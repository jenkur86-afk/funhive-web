...
const STALE_METRICS = {
  nameConformance: {
    value: 68.2, unit: '%', measured: '2026-08-29',
    detail: "573 of 840 distinct scraper_name values conform (57 EXACT + 516 PREFIXED); 267 drift. Conformance rose 60.8% -> 68.2% and the drift count fell 283 -> 267, the second consecutive fall. STILL NOT A REPAIR, and the mechanism is unchanged: the 2026-08-28/29 Group 1 rotation minted new fully-conforming per-site names while old free-text rows aged out of the rolling 2026-08-01 window, growing the denominator 722 -> 840. One name WAS deliberately renamed on 2026-08-29 — LibraryCalendar-Libraries-www -> LibraryCalendar-Libraries-knoxcountylibrary, 12 rows — but 1 of 840 does not move this gate; it is recorded because it is the first renaming performed by any run. Drift classes: FORMAT_DRIFT 64 names / 12010 rows (RecDeskParks-* remains the bulk and still needs RecDesk-Parks-<slug>), FREE_TEXT 141 / 4493, CASE_MISMATCH 25 / 6648 (wordpress-NY alone is 1744 rows), UNRELATED 29 / 1172, BAD_SLUG 8 / 148. COLLAPSED entries: Assabet-NH-MA is now the worst by far at 41 declared sites under ONE name — it is also why that scraper contributes only an aggregate row to LIBRARY-SITE-AUDIT.md — plus CustomDrupal-Libraries 8, CivicEngage-Libraries 2, MacaroniKid-DE 2. The MacaroniKid-MA/AL/VA/SC rows that exceed their declared site counts remain a stale 'sites: N' figure in the registry, not a naming fault.",
