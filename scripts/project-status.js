...
const STALE_METRICS = {
  nameConformance: {
    value: 69.7, unit: '%', measured: '2026-08-30',
    detail: "603 of 865 distinct scraper_name values conform (67 EXACT + 536 PREFIXED); 262 drift. Conformance rose 68.2% -> 69.7% and the drift count fell 267 -> 262, the THIRD consecutive fall. STILL NOT A REPAIR in the bulk: the mechanism remains new fully-conforming per-site names being minted while old free-text rows age out of the rolling window, denominator 840 -> 865. What IS repair this run: three scrapers that had NEVER set metadata.scraperName now do - Intercept-Camden and Drupal-Virginia take the bare registry key, WordPress-Events-Calendar takes a per-site slug across 13 sites - which retires the FREE_TEXT names 'Camden County Library System', 'Handley Regional Library' and 'Library System of Lancaster County' at the next Group 3 rotation. NOT YET RUN, and a rename is not retroactive: existing rows keep the old name until they expire, so both forms will show meanwhile. Drift classes: FORMAT_DRIFT 64 names / 11803 rows (RecDeskParks-* remains the bulk and still needs RecDesk-Parks-<slug>), FREE_TEXT 137 / 4363, CASE_MISMATCH 25 / 6423 (wordpress-NY alone is 1708 rows), UNRELATED 29 / 1091, BAD_SLUG 7 / 99. COLLAPSED entries unchanged: Assabet-NH-MA 41 declared sites under ONE name - documented in that file as NOT fixable from a daily run, because saveEventsWithGeocoding() overwrites metadata.scraperName and verifyAndCleanupEvents() DELETES by that exact value - plus CustomDrupal-Libraries 8, CivicEngage-Libraries 2, MacaroniKid-DE 2.",
