...
const STALE_METRICS = {
  nameConformance: {
    value: 71.7, unit: '%', measured: '2026-09-02',
    detail: "561 of 782 distinct scraper_name values conform (66 EXACT + 495 PREFIXED); 221 drift. The drift count fell 231 -> 221, the FIFTH consecutive fall, while conformance moved only 71.6% -> 71.7%. READ THE DENOMINATOR: it fell 814 -> 782 as old free-text rows aged out of the rolling window, so this movement is again expiry rather than repair — NO RENAME LANDED THIS RUN either, and none is possible from a day with no rotation. Drift classes: FREE_TEXT 108 names / 3955 rows, FORMAT_DRIFT 55 / 7798 (RecDeskParks-* still the bulk, still needs RecDesk-Parks-<slug>), UNRELATED 25 / 953, CASE_MISMATCH 24 / 5944 (wordpress-NY alone is 1629 rows), BAD_SLUG 9 / 189. Worst single names: wordpress-NY 1629, wordpress-NJ 1179, LocalistParks-IN 1053, RecDeskParks-cityofaikensc 821, 'Free Library of Philadelphia' 805. COLLAPSED entries unchanged: Assabet-NH-MA 41 declared sites under ONE name, CustomDrupal-Libraries 8, CivicEngage-Libraries 2, MacaroniKid-DE 2. Worth noting against the drift total: every one of the 106 MacaroniKid per-site identities that wrote today conforms to <registryKey>-<siteSlug>, so the family driving today's only completed run is not part of the problem. PREVIOUS (2026-09-01): 583 of 814 conform; 231 drift. Conformance rose 69.7% -> 71.6% and the drift count fell 262 -> 231, the FOURTH consecutive fall. READ THE DENOMINATOR BEFORE READING THE RATE: it fell 865 -> 814 as old free-text rows aged out of the rolling window, so part of this movement is expiry rather than repair, and no rename landed in this run at all. Drift classes: FREE_TEXT 111 names / 4262 rows, FORMAT_DRIFT 59 / 8081 (RecDeskParks-* is still the bulk and still needs RecDesk-Parks-<slug>), UNRELATED 26 / 1024, CASE_MISMATCH 25 / 6167 (wordpress-NY alone is 1671 rows), BAD_SLUG 10 / 210. Worst single names: wordpress-NY 1671, wordpress-NJ 1216, LocalistParks-IN 1069, RecDeskParks-cityofaikensc 859, 'Free Library of Philadelphia' 846. COLLAPSED entries unchanged: Assabet-NH-MA 41 declared sites under ONE name - documented in that file as NOT fixable from a daily run, because saveEventsWithGeocoding() overwrites metadata.scraperName and verifyAndCleanupEvents() DELETES by that exact value - plus CustomDrupal-Libraries 8, CivicEngage-Libraries 2, MacaroniKid-DE 2. SouthwestGeorgia-GA declares 3 sites and emits 2 names, which is under-coverage rather than a collapse. The three renames logged 2026-08-30 (Intercept-Camden, Drupal-Virginia, WordPress-Events-Calendar) are STILL UNRUN - Group 3 has not rotated since.",
